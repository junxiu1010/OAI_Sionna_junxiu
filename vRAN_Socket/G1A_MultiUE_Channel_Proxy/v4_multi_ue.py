"""
================================================================================
G1A v4_multi_ue.py - Multi-UE Channel Proxy (Socket + GPU IPC dual-mode)
                     Unified Channel Generation

Based on v3_multi_ue.py. Key optimization: unified channel generation.

[Key changes from v3]
  1. Single ChannelProducer with N_UE_sionna=num_ues (was: N per-UE producers)
  2. Single RingBuffer shape (maxlen, N_UE, FFT_SIZE) (was: N buffers of (maxlen, FFT_SIZE))
  3. _H_PDP_FIX and _H_TTI called once for all UEs (was: N times)
  4. GPU kernel launches reduced from N to 1 per channel batch
  5. TF graph compiled once for N-UE tensor (was: N separate traces)

[Architecture — Unified Batched Processing]
  Sionna generates H ∈ ℂ^(N_UE × N_sym × N_FFT) in one GPU call.
  Single RingBuffer stores (N_sym, N_UE, FFT_SIZE) entries.
  TDD reciprocity: H^T = H (SISO), DL/UL share same unified buffer.

  DL: Y_k[s,f] = IFFT( FFT(X_dl[s,f]) ⊙ FFT(H[k,s,f]) )   per-UE slice
  UL: Z[s,f] = Σ_k IFFT( FFT(X_k[s,f]) ⊙ FFT(H[k,s,f]) )  batched sum

[Inherited from v3]
  - DL+UL batch FFT, per-UE logging, CUDA Graph, complex128
...
class ChannelProducer(threading.Thread):
    """Produces channel coefficients for all UEs in a single GPU call.

    v4: N_UE_sionna = num_ues. Output h_delay has shape [N_UE, N_sym, N_FFT]
    after squeeze. Transposed to [N_sym, N_UE, N_FFT] before put_batch so that
    RingBuffer entries are (N_UE, FFT_SIZE).
    """
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            ActiveUE = tf.ones([self.n_ue], dtype=tf.complex64)
            ServingBS = tf.ones([self.params['N_BS']], dtype=tf.complex64)
            ...
            # squeeze singleton dims: [B=1, N_r=1, N_t=1, N_BS=1, N_UE, N_sym, N_FFT]
            # → [N_UE, N_sym, N_FFT] when N_UE>1, or [N_sym, N_FFT] when N_UE=1
            h_squeezed = tf.squeeze(h_delay)
            if self.n_ue == 1:
                h_squeezed = tf.expand_dims(h_squeezed, 0)

            h_c128 = tf.cast(h_squeezed, tf.complex128)
            energy = tf.reduce_sum(tf.abs(h_c128) ** 2, axis=-1, keepdims=True)
            h_norm = h_squeezed / tf.cast(tf.sqrt(energy), h_squeezed.dtype)
            h_c64 = tf.cast(h_norm, tf.complex64)

            # [N_UE, N_sym, N_FFT] → [N_sym, N_UE, N_FFT] for RingBuffer
            h_transposed = tf.transpose(h_c64, [1, 0, 2])

            try:
                h_cp_batch = cp.from_dlpack(tf.experimental.dlpack.to_dlpack(h_transposed)).copy()
            except Exception:
                h_cp_batch = cp.asarray(h_transposed.numpy())
...
class Proxy:
    ...
    def _init_channels(self, buffer_len, buffer_symbol_size):
        """Initialize unified multi-UE Sionna channel model and GPU pipeline.

        v4: Single ChannelProducer with N_UE_sionna=num_ues generates all UE
        channels in one GPU call.  Single RingBuffer with shape (num_ues, FFT_SIZE).
        """
        self.channel_buffer: RingBuffer = None
        self.channel_buffers: List[RingBuffer] = []
        self.producers: List[ChannelProducer] = []
        self._cached_all_channels = None
        ...
        print(f"[G1A-v4] Initializing unified {N_UE_sionna}-UE channel (single producer)...")

        Channel_Generator = ChannelCoefficientsGeneratorJIN(carrier_frequency, scs, ArrayTX, ArrayRX, False)

        tile_ue = [1, 1, N_UE_sionna, 1, 1]
        tau_rays_m = tf.tile(tau_rays, tile_ue)
        phi_r_rays_m = tf.tile(phi_r_rays, tile_ue)
        phi_t_rays_m = tf.tile(phi_t_rays, tile_ue)
        theta_r_rays_m = tf.tile(theta_r_rays, tile_ue)
        theta_t_rays_m = tf.tile(theta_t_rays, tile_ue)
        power_rays_m = tf.tile(power_rays, tile_ue)
        ...
        PDP = Rays(
            delays=tau_rays_m, powers=power_rays_m,
            aoa=phi_r_rays_m, aod=phi_t_rays_m,
            zoa=theta_r_rays_m, zod=theta_t_rays_m, xpr=xpr_pdp
        )

        speeds = [Speed + k * 0.5 for k in range(N_UE_sionna)]
        vel_list = [tf.abs(tf.random.normal(
            [batch_size, 1, 3], mean=s, stddev=0.3, dtype=tf.float32)) for s in speeds]
        velocities = tf.concat(vel_list, axis=1)

        dists = [1.0 + k * 0.3 for k in range(N_UE_sionna)]
        distance_3d = tf.constant([[[d for d in dists]]], dtype=tf.float32)
        ...
        self.channel_buffer = RingBuffer(
            shape=(N_UE_sionna, FFT_SIZE),
            dtype=cp.complex64 if GPU_AVAILABLE else np.complex64,
            maxlen=max(1000, buffer_len)
        )
        self.channel_buffers = [self.channel_buffer]

        params = dict(Fs=Fs, scs=scs, N_UE=N_UE_sionna, N_BS=N_BS,
                      N_UE_active=N_UE_sionna, N_BS_serving=1)

        producer = ChannelProducer(
            self.channel_buffer, Channel_Generator, topology, params,
            h_field_array_power_t, aoa_delay_t, zoa_delay_t,
            buffer_symbol_size=max(32, buffer_symbol_size),
            ue_label="ALL"
        )
        producer.start()
        self.producers.append(producer)

        for k in range(N_UE_sionna):
            print(f"[G1A-v4] UE{k} channel (dist={dists[k]:.1f}, speed={speeds[k]:.1f})")
        print(f"[G1A-v4] Unified producer started: {N_UE_sionna} UEs, "
              f"buffer=({max(1000, buffer_len)}, {N_UE_sionna}, {FFT_SIZE})")
    ...
    def _get_all_channels(self, n_sym):
        """Get channel coefficients for ALL UEs at once from unified buffer.

        Returns shape (N_SYM, num_ues, FFT_SIZE). Caches result so that
        multiple calls within the same slot reuse the same data.
        """
        if self._cached_all_channels is not None:
            return self._cached_all_channels

        if self.channel_buffer is None:
            lib = cp if GPU_AVAILABLE else np
            return lib.ones((N_SYM, self.num_ues, FFT_SIZE), dtype=lib.complex64)

        channels = self.channel_buffer.get_batch(n_sym)
        if n_sym < N_SYM:
            lib = cp if GPU_AVAILABLE else np
            pad = lib.ones((N_SYM - n_sym, self.num_ues, FFT_SIZE), dtype=channels.dtype)
            channels = lib.concatenate([channels, pad])

        self._cached_all_channels = channels
        return channels

    def _invalidate_channel_cache(self):
        """Clear cached channels. Call once per slot boundary."""
        self._cached_all_channels = None

    def _get_channels_for_ue(self, ue_idx, n_sym):
        """Get channel coefficients for a single UE (sliced from unified buffer)."""
        all_ch = self._get_all_channels(n_sym)
        if ue_idx < self.num_ues:
            return all_ch[:, ue_idx, :]
        lib = cp if GPU_AVAILABLE else np
        return lib.ones((N_SYM, FFT_SIZE), dtype=lib.complex64)
    ...
    def _accept_ue(self):
        ...
        ch_label = f"ch={ue_idx}" if ue_idx < self.num_ues else "ch=NONE(>num_ues)"
    ...
    def _handle_gnb_dl(self, ep, hdr_raw, hdr_vals, payload, t_blk0):
        ...
        if (is_channel_slot and self.batch_mode and len(active_ues) > 1
                and all(idx < self.num_ues for idx, _ in active_ues)):
            ...
            for ue_idx, u in active_ues:
                if is_channel_slot and ue_idx < self.num_ues:
    ...
    def _warmup_pipeline(self):
        ...
        print(f"[G1A-v4 IPC] Pipeline ready ({time.time()-t0:.1f}s)")
    ...
    def _ipc_process_dl(self):
        ...
        if is_channel and self.batch_mode and self.num_ues > 1:
            all_ch = self._get_all_channels(n_sym)
            channels_list = [all_ch[:, k, :] for k in range(self.num_ues)]
            gpu_outs = []
        ...
        for ue_k in range(self.num_ues):
            ...
            if is_channel and ue_k < self.num_ues:
                channels = all_ch[:, ue_k, :]
                self.gpu_slot_pipeline.process_slot_ipc(
    ...
    def _ipc_process_ul(self):
        ...
        Batch path: all available full-slot UEs are collected first, then processed
        in a single batched GPU call (process_slot_batch_ul_ipc).
        Sequential fallback: partial-slot or non-channel UEs.
        """
        t0 = time.perf_counter()
        self._invalidate_channel_cache()
        total_int16 = self.gpu_slot_pipeline.total_int16
        expected_int16 = total_int16
        ...
        for ue_k in range(self.num_ues):
            ...
            if (is_full_slot and self.ch_en and self.custom_channel
                    and ue_k < self.num_ues and self.batch_mode):
                n_cpx = n_int16 // 2
                sym_idx = get_ofdm_symbol_indices(n_cpx)
                ul_n_sym = len(sym_idx)
                batch_ue_indices.append(ue_k)
                batch_gpu_ins.append(gpu_in)
            else:
                seq_entries.append((ue_k, gpu_in, n_int16,
                                    is_full_slot and self.ch_en and self.custom_channel
                                    and ue_k < self.num_ues))

        processed_count = len(batch_ue_indices) + len(seq_entries)

        if processed_count == 0:
            return 1000 * (time.perf_counter() - t0)

        if batch_ue_indices and ul_n_sym > 0:
            all_ch = self._get_all_channels(ul_n_sym)
            batch_channels = [all_ch[:, k, :] for k in batch_ue_indices]

        ul_rx_ptr = self.ipc.enqueue_ul_rx(ul_ts, total_int16 // 2, 1, total_int16 * 2)
        if ul_rx_ptr is None:
            for ue_k in batch_ue_indices:
                self.ipc.advance_ul_tx(ue_k)
            for ue_k, _, _, _ in seq_entries:
                self.ipc.advance_ul_tx(ue_k)
            return 1000 * (time.perf_counter() - t0)

        gpu_ul_rx = self.ipc.get_gpu_array(ul_rx_ptr, total_int16 * 2, cp.int16)

        if batch_ue_indices and not seq_entries:
            t_batch = time.perf_counter()
            self.gpu_slot_pipeline.process_slot_batch_ul_ipc(
                batch_gpu_ins, batch_channels, pathLossLinear, snr_dB,
                noise_enabled, gpu_ul_rx)
            batch_ms = 1000 * (time.perf_counter() - t_batch)
            per_ue_ms = batch_ms / len(batch_ue_indices)
            for ue_k in batch_ue_indices:
                self.ipc.advance_ul_tx(ue_k)
                self._e2e_ul_per_ue_ms[ue_k] += per_ue_ms
        else:
            ...
            if batch_ue_indices:
                t_batch = time.perf_counter()
                tmp_out = cp.zeros(total_int16, dtype=cp.int16)
                self.gpu_slot_pipeline.process_slot_batch_ul_ipc(
                    batch_gpu_ins, batch_channels, pathLossLinear, snr_dB,
                    noise_enabled, tmp_out)
                f64 = tmp_out.astype(cp.float64)
                gpu_accum += f64[::2] + 1j * f64[1::2]
                batch_ms = 1000 * (time.perf_counter() - t_batch)
                per_ue_ms = batch_ms / len(batch_ue_indices)
                for ue_k in batch_ue_indices:
                    self.ipc.advance_ul_tx(ue_k)
            ...
            for ue_k, gpu_in, n_int16, use_channel in seq_entries:
                t_ue = time.perf_counter()
                if use_channel:
                    n_cpx = n_int16 // 2
                    sym_idx = get_ofdm_symbol_indices(n_cpx)
