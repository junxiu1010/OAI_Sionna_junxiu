...
class GPUSlotPipeline:
    ...
    def process_slot_ipc(self, gpu_iq_in_arr, channels_gpu, pl_linear, snr_db, noise_on, gpu_iq_out_arr):
        ...
        with self.stream:
            # GPU→GPU copy from IPC buffer to working buffer
            ...
            # GPU→GPU copy result to IPC output buffer
        ...
        if do_profile:
            mode = "GRAPH" if self.graph_captured else "NORMAL"
            print(f"\n[PROFILE IPC slot#{self.slot_counter} {mode}] "
                  f"GPU_COPY_IN={1000*(t1-t0):.2f}ms "
                  f"CH_COPY={1000*(t2-t1):.2f}ms "
                  f"GPU_COMPUTE={1000*(t3-t2):.2f}ms "
                  f"GPU_COPY_OUT={1000*(t4-t3):.2f}ms "
                  f"TOTAL={1000*(t4-t0):.2f}ms\n")
...
class Proxy:
    ...
    def _process_ofdm_slot(self, iq_bytes, ts):
        ...
        return self.gpu_slot_pipeline.process_slot(
    ...
    def _ipc_process_dl(self):
        ...
        The gNB writes variable-sized chunks: full slots (30720 samples),
        partial slots (e.g. 13184 samples for TDD flexible slot DL portion),
        or slightly oversized writes. Only full slots go through the OFDM
        channel pipeline; everything else is passed through unchanged.
        """
        ...
        expected_int16 = self.gpu_slot_pipeline.total_int16  # 61440 for 1 NR slot
        ...
        try:
            while True:
                ...
                if self.ipc.get_ul_tx_ready() == 1:
                    self._ipc_process_ul()
                    ul_count += 1
                    processed = True

                if not processed:
                    time.sleep(0.0001)  # 100us idle sleep
