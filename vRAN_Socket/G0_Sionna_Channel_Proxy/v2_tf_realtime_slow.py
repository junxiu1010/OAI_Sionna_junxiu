"""
================================================================================
v2_tf_realtime_slow.py - TensorFlow 실시간 Sionna 채널 생성
...
class Proxy:
    ...
    def process_ofdm_slot_with_custom_channel(self, iq_bytes, ts, log_plot=False,
        ...
        h_delay = h_delay / np.sqrt(np.sum(np.abs(h_delay)**2))
        out = np.zeros_like(x_cpx)
        symbol_waveforms = []
        out_waveforms = []
        ...
        for n, (start, end) in enumerate(sym_idx[:N_symbols]):
            ...
            if end - start != FFT_SIZE + cp_len:
                continue
            sym = x_cpx[start + cp_len: end]
            h_this = h_delay[n].numpy() if hasattr(h_delay[n], 'numpy') else h_delay[n]
            if conv_mode == "fft":
