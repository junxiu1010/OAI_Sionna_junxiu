"""
================================================================================
v4_tf_ringbuffer_copy.py - TensorFlow + RingBuffer 비동기 채널 생성 (복사본)

[성능] ✅ ~92ms (10 DL당) | 실시간 대비 7.7배 느림
[속도 원인] RingBuffer로 채널 생성을 비동기 처리하나 TensorFlow 오버헤드 존재

[핵심 특징]
- GPU: TensorFlow
- 채널: 백그라운드 스레드에서 비동기 생성
- 버퍼: RingBuffer로 채널 데이터 큐잉
- v4 복사본

[중간 속도 이유]
- 비동기 채널 생성으로 blocking 감소 (개선)
- TensorFlow FFT 오버헤드 (느림)
- Pinned Memory 미사용 (느림)
================================================================================
"""
...
class Proxy:
    def __init__(self, ue_port, gnb_host, gnb_port, log_level, ch_en=True, ch_L=32, ch_dd=0, log_plot=False,
        ...
        # --- External sync pacing (optional, align with OAI EXTSYNC env) ---
        self.extsync = int(os.environ.get("EXTSYNC", "0"))
        self.extsync_offset_us = int(os.environ.get("EXTSYNC_OFFSET_US", "0"))
        self.extsync_mode = os.environ.get("EXTSYNC_MODE", "")
        self.slots_per_frame = 20  # numerology 1
        # allow override via EXTSYNC_SLOT_US (us). default 10ms/slots_per_frame
        slot_us_env = os.environ.get("EXTSYNC_SLOT_US", "")
        if slot_us_env.isdigit() and int(slot_us_env) > 0:
            self.slot_period_ns = int(slot_us_env) * 1000
        else:
            self.slot_period_ns = int(10_000_000 / self.slots_per_frame)
        # local tick(default) or IPC
        self.next_tick_ns = None
        self.sync_sock = None
        if self.extsync_mode == "ipc":
            port = int(os.environ.get("EXTSYNC_UDP_PORT", "55555"))
            maddr = os.environ.get("EXTSYNC_MCAST", "")
            s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            s.bind(("", port))
            # optional recv timeout to avoid hard-stall if tick is missing
            try:
                timeout_ms = int(os.environ.get("EXTSYNC_RECV_TIMEOUT_MS", "200"))
            except ValueError:
                timeout_ms = 200
            if timeout_ms > 0:
            ...
            if maddr:
                try:
                    first_octet = int(maddr.split('.')[0])
                except Exception:
                    first_octet = 0
                # Join only if maddr is multicast (224.0.0.0/4). For unicast (e.g., 127.0.0.1), skip membership.
            ...
            self.next_tick_ns = now + self.extsync_offset_us * 1000
        # limited debug
        try:
            self._ext_dbg_limit = int(os.environ.get("EXTSYNC_DEBUG_LIMIT", "50"))
        except ValueError:
            self._ext_dbg_limit = 50
        self._ext_dbg_count = 0
    def _wait_tick(self):
        """Wait until the next slot tick if external sync is enabled.
        Supports two modes: ipc(UDP absolute ns), or local EXTSYNC tick."""
        # IPC mode: receive absolute monotonic ns
        if self.sync_sock is not None:
            try:
                data, _ = self.sync_sock.recvfrom(8)
            except socket.timeout:
                if self._ext_dbg_count < self._ext_dbg_limit:
                    print("[EXTSYNC][Sionna] tick timeout")
                    self._ext_dbg_count += 1
                return
            if len(data) == 8:
                (next_ns,) = struct.unpack("=Q", data)
                if self._ext_dbg_count < self._ext_dbg_limit:
                    print(f"[EXTSYNC][Sionna] tick next_ns={next_ns}")
                    self._ext_dbg_count += 1
                while True:
                    now_ns = time.monotonic_ns()
                    if now_ns >= next_ns:
                        break
                    remain = max((next_ns - now_ns)/1e9, 0)
                    time.sleep(min(remain, 0.0002))
            return
        # Local EXTSYNC pacing
        ...
        while True:
            ...
            if now_ns >= self.next_tick_ns:
                break
            remain = max((self.next_tick_ns - now_ns)/1e9, 0)
            time.sleep(min(remain, 0.0002))
        self.next_tick_ns += self.slot_period_ns
    ...
    def _handle_ep(self,ep:Endpoint):
        for hdr_raw, hdr_vals, payload in ep.read_blocks():
            # external sync pacing (slot granularity approximate)
            self._wait_tick()
