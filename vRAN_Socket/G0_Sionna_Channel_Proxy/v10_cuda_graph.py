...
class GPUSlotPipeline:
    ...
    Version 10: CUDA Graph Pipeline (complex128)
    ...
    def __init__(self, fft_size=2048, enable_gpu=True, use_pinned_memory=True, use_cuda_graph=True):
        ...
        print(f"[GPU Pipeline v10] CUDA Graph + complex128 Pipeline 초기화 중...")
        print(f"[GPU Pipeline v10] Pinned Memory: {'활성화' if self.use_pinned_memory else '비활성화'}")
        print(f"[GPU Pipeline v10] CUDA Graph: {'활성화' if use_cuda_graph else '비활성화'}")
        print(f"[GPU Pipeline v10] 정밀도: complex128 (float64 기반, PSS 검출 안정성 확보)")
        ...
        print(f"[GPU Pipeline v10] 인덱스 사전계산 완료 (sym={self.n_sym}, fft={fft_size})")
        ...
        if self.use_pinned_memory:
            ...
            print(f"[GPU Pipeline v10] Pinned Memory: {mem_mb:.2f} MB "
        ...
        print(f"[GPU Pipeline v10] GPU Memory: {gpu_mb:.2f} MB (complex128, v9과 동일)")
        ...
        print(f"[GPU Pipeline v10] 초기화 완료 - CUDA Graph + complex128 Pipeline")
        print(f"[GPU Pipeline v10] H2D: {self.total_int16*2//1024}KB(int16) → "
              f"GPU처리(complex128) → D2H: {self.total_int16*2//1024}KB(int16)")
        if use_cuda_graph:
            print(f"[GPU Pipeline v10] CUDA Graph: 워밍업 {self.WARMUP_SLOTS}슬롯 후 캡처")
    ...
    def process_slot(self, iq_bytes, channels_gpu, pl_linear, snr_db, noise_on):
        ...
        if do_profile:
            ...
            print(f"\n[PROFILE slot#{self.slot_counter} {mode}] "
                  f"H2D={1000*(t1-t0):.2f}ms "
                  f"CH_COPY={1000*(t2-t1):.2f}ms "
                  f"GPU_COMPUTE={1000*(t3-t2):.2f}ms "
                  f"D2H={1000*(t4-t3):.2f}ms "
...
class Proxy:
    def __init__(self, ue_port, gnb_host, gnb_port, log_level, ch_en=True, ch_L=32, ch_dd=0, log_plot=False,
        ...
        if self.custom_channel:
            ...
            print(f"[INFO] GPU Slot Pipeline v10 초기화 완료 "
        ...
        for hdr_raw, hdr_vals, payload in ep.read_blocks():
            ...
            if size > 1 and self.ch_en:
                if self.custom_channel:
                    ...
                    ch_note = f" (GPU slot pipeline v10)"
                else:
                    processed = payload
                    ch_note = ""
            else:
    ...
    def process_ofdm_slot_with_slot_pipeline(self, iq_bytes, ts, log_plot=False):
        """
        Version 10: CUDA Graph + complex128 Pipeline (bytes → bytes)
        """
...
def main():
    ap = argparse.ArgumentParser(description="vRAN Socket Sionna Channel Simulator - Version 10 (CUDA Graph Pipeline)")
    ...
    print("=" * 80)
    print("vRAN Socket Channel Simulator - Version 10: CUDA Graph Pipeline")
    print("=" * 80)
    ...
    if args.enable_gpu and GPU_AVAILABLE:
        ...
        # v10 추가 최적화
        print("  ✓ [v10] complex128 정밀도 유지 (PSS/SSS 검출 안정성)")
        if args.use_cuda_graph:
            print(f"  ✓ [v10] CUDA Graph (워밍업 {GPUSlotPipeline.WARMUP_SLOTS}슬롯 후 캡처, "
                  f"~15 커널 → 1 graph launch)")
        else:
            print("  ✗ [v10] CUDA Graph 비활성화 (일반 커널 런치)")
    else:
