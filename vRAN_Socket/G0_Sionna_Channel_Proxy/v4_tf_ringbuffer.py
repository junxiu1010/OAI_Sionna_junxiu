"""
================================================================================
v4_tf_ringbuffer.py - TensorFlow + RingBuffer 비동기 채널 생성

[성능] ✅ ~85ms (10 DL당) | 실시간 대비 7.1배 느림
...
class Proxy:
    ...
    def _start_api_server(self):
        ...
        api_sock.bind((SIONNA_API_IP, SIONNA_API_PORT))
        api_sock.listen(1)
        print(f"[Sionna API] Server listening on {SIONNA_API_IP}:{SIONNA_API_PORT}")

        while True:
            conn, addr = api_sock.accept()
            print(f"[Sionna API] Client connected from {addr}")
            client_handler = threading.Thread(target=self._handle_api_client, args=(conn,))
            client_handler.daemon = True
            client_handler.start()

    def _handle_api_client(self, conn):
        try:
            while True:
                data = conn.recv(4096)
                if not data: break
                ...
                try:
                    ...
                    response = self._process_api_command(message)
            ...
            print(f"[Sionna API] Error handling client: {e}")
        finally:
            conn.close()
            print(f"[Sionna API] Client disconnected.")

    def _process_api_command(self, command):
        cmd_type = command.get("type")
        if cmd_type == "GET_PARAMS":
            return {"status": "success", "params": self.sionna_params}
        elif cmd_type == "SET_PARAM":
            param_name = command.get("name")
            param_value = command.get("value")
            if param_name and param_value is not None:
                try:
                    if hasattr(self, param_name):
                        setattr(self, param_name, param_value)
                        self.sionna_params[param_name] = param_value # 딕셔너리도 업데이트
                        print(f"[Sionna API] Set {param_name} to {param_value}")
                        return {"status": "success", "message": f"Parameter {param_name} updated."}
                    else:
                        return {"status": "error", "message": f"Unknown parameter: {param_name}"}
                except Exception as e:
                    return {"status": "error", "message": f"Failed to set parameter {param_name}: {e}"}
            else:
                return {"status": "error", "message": "Missing parameter name or value."}
        else:
