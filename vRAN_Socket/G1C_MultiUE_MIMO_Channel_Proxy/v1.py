...
[G1B v8 → G1C v0 변경사항]
  ...
  4. UL Superposition: UE[k] ul_tx → per-UE 채널 적용 → 합산 → gNB ul_rx
  5. UL Bypass: 채널 미적용 시 각 UE sequential copy (마지막 UE 우선)
  6. Per-UE 독립 리소스: pipeline, channel buffer, noise producer
...
class Proxy:
    ...
    def _init_channel(self, buffer_len, buffer_symbol_size):
        ...
        if not self.custom_channel:
            print(f"[G1C] Bypass mode — {N} UE(s), no channel")
            return
        ...
        print(f"[G1C] Multi-UE Channel Proxy initialized: {N} UE(s), "
    ...
    def _warmup_pipeline(self):
        ...
        if not self.custom_channel:
            ...
            return
        print(f"[G1C] Pre-warming {self.num_ues} UE(s) DL/UL pipelines...")
        t0 = time.time()
        ...
        print(f"[G1C] All {self.num_ues} UE(s) pipelines ready ({time.time()-t0:.1f}s)")
    ...
    def run_ipc(self):
        ...
        print(f"[G1C] Entering main loop ({N} UE(s), gnb_ant={self.gnb_ant}, ue_ant={self.ue_ant})...")
        dl_count = 0
        ul_count = 0
        t_start = time.time()
        proxy_dl_head = 0
