"""
================================================================================
v3.py - G1C Multi-UE MIMO Channel Proxy (v2 + P1B per-UE ray + stall detection)

[v2 → v3 변경사항]
  1. UE별 독립 P1B Ray 데이터: --p1b-npz로 P1B npz 지정, per-UE RX 위치 선택
     - degree→radian 변환 후 ChannelProducerProcess에 ray_data dict로 전달
     - --ue-rx-indices: 수동 지정, "random", 또는 미지정 시 자동 random
     - 무효 인덱스 시 가장 가까운 유효 인덱스 안내
  2. Active-set 기반 UE stall 감지:
     - max_head - head_k < cir_time 조건으로 active/stalled 분류
     - stalled UE를 superposition에서 자동 제외 (1회 경고 로깅)
     - E2E per-UE UL 카운터: active UE만 증가
  3. _ipc_ul_combine / _ipc_ul_superposition_slot: active_ues 파라미터 추가

[v2에서 유지]
  - multiprocessing.Process 기반 ChannelProducer
  - IPCRingBuffer (CUDA IPC cross-process)
  - IPC V7 + futex, UL fused RawKernel, CUDA Graph

[아키텍처]
  P1B npz → load_p1b_per_ue() → per-UE ray_data (radian)
  ChannelProducerProcess[k] (독립 프로세스: TF+CuPy, 독립 ray)
...
class ChannelProducerProcess(_mp_ctx.Process):
    ...
    def run(self):
        ...
        if 'ray_data' in cfg:
            phi_r_rays = _tf.convert_to_tensor(cfg['ray_data']['phi_r'])
            phi_t_rays = _tf.convert_to_tensor(cfg['ray_data']['phi_t'])
            theta_r_rays = _tf.convert_to_tensor(cfg['ray_data']['theta_r'])
            theta_t_rays = _tf.convert_to_tensor(cfg['ray_data']['theta_t'])
            power_rays = _tf.convert_to_tensor(cfg['ray_data']['power'])
            tau_rays = _tf.convert_to_tensor(cfg['ray_data']['tau'])
            print(f"[v3 ChannelProducerProcess UE[{ue_idx}]] P1B ray_data loaded (radian)")
        else:
        ...
        ArrayRX = PanelArray(
            ...
            num_rows=1, num_cols=1,
            polarization=pol_mode, polarization_type=pol_type, antenna_pattern='omni',
            carrier_frequency=carrier_freq)

        print(f"[v3 ChannelProducerProcess UE[{ue_idx}]] Sionna init start")
        ...
        print(f"[v3 ChannelProducerProcess UE[{ue_idx}]] Sionna init done, allocating GPU ring buffer")

        shape = cfg['shape']
        rb = IPCRingBufferProducer(shape, _cp.complex128, self.sync)
        self.handle_queue.put(rb.ipc_handle)
        print(f"[v3 ChannelProducerProcess UE[{ue_idx}]] IPC handle sent, entering generation loop")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                wideband_energy = float(_cp.mean(_cp.abs(h_cp_batch) ** 2))
                if wideband_energy > 0:
                    h_cp_batch = h_cp_batch / _cp.sqrt(_cp.float64(wideband_energy))

                rb.put_batch(h_cp_batch)
                symbol_counter += buffer_symbol_size

            except _tf.errors.ResourceExhaustedError as e:
                print(f"[v3 ChannelProducerProcess UE[{ue_idx}]] GPU OOM — stopping: {e}")
                break
            except Exception as e:
                print(f"[v3 ChannelProducerProcess UE[{ue_idx}]] ERROR in generation loop: {e}")
                import traceback
                traceback.print_exc()
                time.sleep(1.0)
        ...
        try:
            ...
            pass
        print(f"[v3 ChannelProducerProcess UE[{ue_idx}]] Stopped (symbols={symbol_counter})")
...
class Proxy:
    ...
    def _init_channel(self, buffer_len, buffer_symbol_size):
        ...
        for k in range(N):
            ...
            print(f"[v3] UE[{k}] pipelines created (DL: {self.gnb_ant}tx→{self.ue_ant}rx, UL: {self.ue_ant}tx→{self.gnb_ant}rx)")
        ...
        if not self.custom_channel:
            print(f"[v3] Bypass mode — {N} UE(s), no channel")
            return
        ...
        print(f"[v3] Starting {N} ChannelProducerProcess(es) via spawn...")
        ...
        for k in range(N):
            ...
            if self.p1b_npz and self.ue_rx_indices:
                config_k['ray_data'] = load_p1b_per_ue(
                    self.p1b_npz, self.ue_rx_indices[k])
                print(f"[v3] UE[{k}] P1B ray loaded: RX{self.ue_rx_indices[k]}")

            proc_k = ChannelProducerProcess(k, config_k, sync_k, handle_q_k, stop_k)
            proc_k.start()
            print(f"[v3] UE[{k}] ChannelProducerProcess started (pid={proc_k.pid})")

            try:
                ipc_handle = handle_q_k.get(timeout=120)
            except Exception as e:
                print(f"[v3] ERROR: UE[{k}] IPC handle not received within 120s: {e}")
                if proc_k.is_alive():
                    proc_k.terminate()
            ...
            print(f"[v3] UE[{k}] IPCRingBufferConsumer connected (CUDA IPC)")

        self.channel_buffer = self.channel_buffers[0]
        self.producer = self.channel_producers[0]

        print(f"[v3] Multi-UE Channel Proxy initialized: {N} UE(s), "
    ...
    def _warmup_pipeline(self):
        ...
        for np_thread in self.noise_producers:
            np_thread.start()
            print(f"[v3] NoiseProducer started (noise_len={np_thread.noise_len}, batch={np_thread.BATCH_SIZE})")

        if not self.custom_channel:
            print("[v3] Bypass mode — channel warmup skipped")
            return
        print(f"[v3] Pre-warming {self.num_ues} UE(s) DL/UL pipelines...")
        t0 = time.time()
        ...
        for k in range(self.num_ues):
            if self.channel_producers and hasattr(self.channel_producers[k], 'is_alive'):
                if not self.channel_producers[k].is_alive():
                    print(f"[v3 WARN] ChannelProducerProcess[{k}] died during warmup")
        ...
        print(f"[v3] All {self.num_ues} UE(s) pipelines ready ({time.time()-t0:.1f}s)")
    ...
    def run_ipc(self):
        ...
        print(f"[v3] Entering main loop ({N} UE(s), gnb_ant={self.gnb_ant}, ue_ant={self.ue_ant})...")
        dl_count = 0
        ul_count = 0
        t_start = time.time()
        proxy_dl_head = 0
        ...
        try:
            ...
            print(f"\n[v3] Terminated by Ctrl-C (DL: {dl_count}, UL: {ul_count}, UEs: {N})")
        finally:
    ...
    def _cleanup_channel_producers(self):
        ...
        for i, proc in enumerate(getattr(self, 'channel_producers', [])):
            if hasattr(proc, 'join'):
                proc.join(timeout=3)
                if proc.is_alive():
                    print(f"[v3] ChannelProducerProcess[{i}] did not exit gracefully, killing")
                    proc.kill()
                    proc.join(timeout=2)
        ...
        print("[v3] All ChannelProducerProcesses cleaned up")
...
def main():
    ...
    ap.add_argument("--channel-mode", choices=["dynamic", "static"], default="dynamic",
                    help="Channel generation mode (v3 always uses dynamic, accepted for compatibility)")
    ...
    if args.p1b_npz and not os.path.isabs(args.p1b_npz):
        ...
        print(f"[v3] P1B npz path resolved: {args.p1b_npz}")
    ...
    print("=" * 80)
    print("G1C v3 Multi-UE MIMO Channel Proxy (P1B ray + stall detection)")
    print("=" * 80)
