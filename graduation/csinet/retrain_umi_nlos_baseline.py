#!/usr/bin/env python3
"""
UMi_NLOS Baseline CsiNet 재학습 — 데이터 품질 개선 적용
========================================================
원본 preprocessed 데이터의 대부분 샘플이 near-zero power 문제.
→ 원본 raw dataset에서 power-기준 필터링 후 재전처리 & 학습.
"""
...
def preprocess_with_filtering(raw_path, power_percentile=30):
    """
    Raw dataset -> angular-delay -> power 기준 필터링 -> 정규화.
    하위 power_percentile% 샘플 제거.
    """
    ...
    for name, H_raw in [("train", H_train_raw), ("val", H_val_raw), ("test", H_test_raw)]:
        ...
        print(f"  {name}: {len(H_raw)} -> {len(H_ad_filtered)} (kept {100*mask.sum()/len(mask):.1f}%)")
        ...
        print(f"    X power after norm: min={x_power.min():.4f}, max={x_power.max():.4f}, "
              f"mean={x_power.mean():.4f}")
...
def train_one(gamma, X_train, X_val, X_test):
    M = max(1, int(2 * NT * NC_PRIME * gamma))
    tag = f"{SCENARIO}_gamma{gamma:.4f}"
    print(f"\n{'='*60}", flush=True)
    print(f"  Training: {SCENARIO}, gamma={gamma:.4f}, M={M}", flush=True)
...
if __name__ == "__main__":
    raw_path = "/workspace/graduation/csinet/datasets/dataset_UMi_NLOS.h5"

    print(f"=== Re-preprocessing {SCENARIO} with power filtering ===", flush=True)
    X_train, X_val, X_test = preprocess_with_filtering(raw_path, power_percentile=30)

    save_path = "/workspace/graduation/csinet/datasets/preprocessed_UMi_NLOS_filtered.h5"
    ...
    print(f"Saved filtered data: {save_path}", flush=True)

    all_results = []
    for gamma in COMPRESSION_RATIOS:
        r = train_one(gamma, X_train, X_val, X_test)
        all_results.append(r)

    result_path = os.path.join(CKPT_DIR, f"retrain_{SCENARIO}_results.json")
    with open(result_path, "w") as f:
        json.dump(all_results, f, indent=2)

    print("\n\n=== Retraining Summary ===", flush=True)
    for r in all_results:
        print(f"  gamma={r['gamma']:.4f} M={r['M']:3d} | "
              f"NMSE={r['test_nmse_dB']:.2f} dB, cos={r['test_cos_sim']:.4f}", flush=True)

    nmses = [r['test_nmse_dB'] for r in all_results]
    is_monotonic = all(nmses[i] <= nmses[i+1] for i in range(len(nmses)-1))
    print(f"Monotonic check: {'PASS' if is_monotonic else 'FAIL'}", flush=True)
