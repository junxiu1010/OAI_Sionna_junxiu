...
// 글로벌 모드 변수 (1: OAI 채널, 2: 채널 우회)
static int channel_mode = 2;

__attribute__((constructor))
static void init_channel_mode() {
  char *env = getenv("OAI_CHANNEL_MODE");
  if (env) {
    int val = atoi(env);
    if (val == 1 || val == 2) channel_mode = val;
  }
}
                ...
                bool add_noise)
{
  if (channel_mode == 2) {
    ...
    for (int i = 0; i < nbSamples; i++) {
      after_channel_sig[i].r = input_sig[i].r;
      after_channel_sig[i].i = input_sig[i].i;
    }
    return;
  }
  ...
  if (channelDesc->Doppler_phase_cur == NULL) {
    // 배열이 없으면 건너뛴다
    return;
  }
