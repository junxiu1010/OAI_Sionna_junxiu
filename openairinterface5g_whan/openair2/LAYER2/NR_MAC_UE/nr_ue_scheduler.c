    ...
    if (csi_request.val & (1 << i)) {
      ...
      for (int c = 0; c < csi_measconfig->csi_ReportConfigToAddModList->list.count; c++) {
        ...
        if (report_config->reportConfigId == id) {
          ...
          if (tda >= offset_list->list.count) {
            LOG_W(NR_MAC, "TDA index from DCI %d exceeds slot offset list %d (PDCCH false positive)\n", tda, offset_list->list.count);
...
void nr_ue_pucch_scheduler(NR_UE_MAC_INST_t *mac, frame_t frameP, int slotP)
{
  ...
  if (mac->state == UE_CONNECTED) {
    ...
    if (csi_res > 0) {
      LOG_I(NR_MAC, "CSI: Scheduling CSI report in frame %d slot %d, csi_res=%d\n", frameP, slotP, csi_res);
    }
  }
  if (csi_res > 0) {
    num_res += csi_res;
  ...
  if (num_res == 0) {
    LOG_D(NR_MAC, "PUCCH: No resources to transmit in frame %d slot %d\n", frameP, slotP);
    return;
  }
  // do no transmit pucch if only SR scheduled and it is negative
  if (num_res == 1 && pucch[0].n_sr > 0 && pucch[0].sr_payload == 0) {
    LOG_D(NR_MAC, "PUCCH: Only negative SR, skipping transmission in frame %d slot %d\n", frameP, slotP);
    return;
  }
    
  LOG_I(NR_MAC, "PUCCH: Transmitting %d resources in frame %d slot %d\n", num_res, frameP, slotP);
...
uint8_t set_csirs_measurement_bitmap(NR_CSI_MeasConfig_t *csi_measconfig, NR_CSI_ResourceConfigId_t csi_res_id)
{
  ...
  for(int i = 0; i < csi_measconfig->csi_ReportConfigToAddModList->list.count; i++) {
    ...
    switch (report_config->reportQuantity.present) {
      ...
      case NR_CSI_ReportConfig__reportQuantity_PR_cri_RI_LI_PMI_CQI:
        meas_bitmap |= (1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4);
        break;
      default:
  ...
  // 강제로 모든 측정 활성화 (디버그용)
  if (meas_bitmap == 1) {
    meas_bitmap = 0x1e; // RSRP + RI + PMI + CQI (표준 케이스)
    LOG_I(NR_MAC, "Forcing standard CSI measurements: 0x%02x\n", meas_bitmap);
  }
  
  // 디버그 로그 추가
  LOG_I(NR_MAC, "CSI Measurement Bitmap: 0x%02x (RSRP:%d, RI:%d, LI:%d, PMI:%d, CQI:%d, i1:%d)\n", 
        meas_bitmap, 
        (meas_bitmap & 1) ? 1 : 0,
        (meas_bitmap & 2) ? 1 : 0,
        (meas_bitmap & 4) ? 1 : 0,
...
void nr_schedule_csirs_reception(NR_UE_MAC_INST_t *mac, int frame, int slot)
{
  ...
  for (int id = 0; id < csi_measconfig->nzp_CSI_RS_ResourceToAddModList->list.count; id++) {
    ...
    // Propagate Type-II Port Selection config to PHY if any CSI report uses it
    csirs_config_pdu->type2_port_selection = false;
    for (int ri = 0; ri < MAX_CSI_REPORTCONFIG; ri++) {
      if (mac->csi_report_template[ri].is_type2_port_selection) {
        csirs_config_pdu->type2_port_selection = true;
        csirs_config_pdu->type2_num_beams = mac->csi_report_template[ri].type2_num_beams;
        csirs_config_pdu->type2_port_sel_d = mac->csi_report_template[ri].type2_port_sel_d;
        csirs_config_pdu->type2_phase_alphabet = mac->csi_report_template[ri].type2_phase_alphabet;
        csirs_config_pdu->type2_subband_amplitude = mac->csi_report_template[ri].type2_subband_amplitude;
        break;
      }
    }
