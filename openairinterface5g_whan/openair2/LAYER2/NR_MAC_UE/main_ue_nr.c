...
void nr_ue_init_mac(NR_UE_MAC_INST_t *mac)
{
  ...
  // Default subband phase per UE for Type-II PMI diversity before first CSI-RS
  // Each UE runs as a separate process (ue_id always 0), so use PID for uniqueness
  mac->csirs_measurements.num_subbands = 1;
  uint32_t seed = (uint32_t)((uint32_t)getpid() * 2654435761U);
  for (int c = 0; c < 2 * FAPI_NR_TYPE2_MAX_BEAMS; c++) {
    for (int lay = 0; lay < FAPI_NR_TYPE2_MAX_LAYERS; lay++) {
      mac->csirs_measurements.subband_phase[0][c][lay] = (seed >> (c * 2 + lay)) & 0x3;
    }
  }
  LOG_I(NR_MAC, "[UE%d pid=%d] Default subband phases: c0=%d c1=%d c2=%d c3=%d\n",
        mac->ue_id, (int)getpid(),
        mac->csirs_measurements.subband_phase[0][0][0],
        mac->csirs_measurements.subband_phase[0][1][0],
        mac->csirs_measurements.subband_phase[0][2][0],
        mac->csirs_measurements.subband_phase[0][3][0]);
}
...
void release_mac_configuration(NR_UE_MAC_INST_t *mac, NR_UE_MAC_reset_cause_t cause)
{
  ...
  // Restore default subband phase after memset (PID-based for per-process diversity)
  mac->csirs_measurements.num_subbands = 1;
  uint32_t seed_r = (uint32_t)((uint32_t)getpid() * 2654435761U);
  for (int c = 0; c < 2 * FAPI_NR_TYPE2_MAX_BEAMS; c++)
    for (int lay = 0; lay < FAPI_NR_TYPE2_MAX_LAYERS; lay++)
      mac->csirs_measurements.subband_phase[0][c][lay] = (seed_r >> (c * 2 + lay)) & 0x3;
}
