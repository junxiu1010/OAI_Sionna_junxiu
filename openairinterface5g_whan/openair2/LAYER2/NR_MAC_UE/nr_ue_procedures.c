    ...
    if (csi_resourceconfig->csi_ResourceConfigId == csi_ResourceConfigId) {
      ...
      for (int csi_idx = 0; csi_idx < csi_MeasConfig->nzp_CSI_RS_ResourceSetToAddModList->list.count; csi_idx++) {
        if (csi_MeasConfig->nzp_CSI_RS_ResourceSetToAddModList->list.array[csi_idx]->nzp_CSI_ResourceSetId ==
          ...
          // Type-II Port Selection: pack WB PMI into a flat bitstream
          // Use config flag only; PHY measurement flag may not be set before first CSI-RS
          if (csi_report->is_type2_port_selection) {
            int ri = mac->csirs_measurements.rank_indicator;
            int num_layers = ri + 1;
            int L = csi_report->type2_num_beams;
            int d = csi_report->type2_port_sel_d;
            int P = (csi_report->N1 > 0 ? csi_report->N1 : 2) * (csi_report->N2 > 0 ? csi_report->N2 : 1) * 2;
            int ps_bits = 0;
            { int tmp = P / d; while ((1 << ps_bits) < tmp) ps_bits++; }
            int sc_bits = 0;
            { int tmp = 2 * L; while ((1 << sc_bits) < tmp) sc_bits++; }
            int total_coeffs = 2 * L;
            if (total_coeffs > 2 * d) total_coeffs = 2 * d;

            int num_sb = mac->csirs_measurements.num_subbands;
            ...
            int phase_alphabet = csi_report->type2_phase_alphabet;
            static const double psk8_ph[8] = {0.0, M_PI / 4, M_PI / 2, 3 * M_PI / 4,
                                              M_PI, 5 * M_PI / 4, 3 * M_PI / 2, 7 * M_PI / 4};

            // Compute averaged phase indices per (coeff, layer)
            uint8_t avg_phase[2 * FAPI_NR_TYPE2_MAX_BEAMS][FAPI_NR_TYPE2_MAX_LAYERS];
            memset(avg_phase, 0, sizeof(avg_phase));
            for (int lay = 0; lay < num_layers && lay < FAPI_NR_TYPE2_MAX_LAYERS; lay++) {
              for (int c = 0; c < total_coeffs; c++) {
                if (use_fallback) {
                  avg_phase[c][lay] = fallback_phase[c][lay];
                } else {
                  double sum_re = 0.0, sum_im = 0.0;
                  for (int sb = 0; sb < num_sb; sb++) {
                    int idx = mac->csirs_measurements.subband_phase[sb][c][lay];
                    double ph;
                    if (phase_alphabet == 8) ph = psk8_ph[idx & 0x7];
                    else                     ph = qpsk_ph[idx & 0x3];
                    sum_re += cos(ph);
                    sum_im += sin(ph);
                  }
            ...
            // Pack sub-fields in REVERSE order so that after the global
            // reverse_bits() the gNB's pickandreverse_bits() reads them
            // in the expected forward order.
            uint64_t wb_pmi = 0;
            int wb_bits = 0;
            ...
            // 2) has_phase flag (1 bit)
            int has_phase = 1;
            wb_pmi |= ((uint64_t)has_phase) << wb_bits;
            wb_bits += 1;

            // 3) WB amplitudes, reverse loop order
            for (int lay = nlim - 1; lay >= 0; lay--) {
              for (int c = total_coeffs - 1; c >= 0; c--) {
                if (c == mac->csirs_measurements.strongest_coeff_indicator[lay]) continue;
                wb_pmi |= ((uint64_t)(mac->csirs_measurements.wideband_amplitude[c][lay] & 0x7)) << wb_bits;
                wb_bits += 3;
              }
            }

            // 4) Strongest coefficient indicators, reverse layer order
            for (int lay = nlim - 1; lay >= 0; lay--) {
              wb_pmi |= ((uint64_t)(mac->csirs_measurements.strongest_coeff_indicator[lay] & ((1 << sc_bits) - 1))) << wb_bits;
              wb_bits += sc_bits;
            }

            // 5) Port selection indicator (gNB reads FIRST → pack LAST at MSB)
            wb_pmi |= ((uint64_t)mac->csirs_measurements.port_selection_indicator & ((1 << ps_bits) - 1)) << wb_bits;
            wb_bits += ps_bits;

            LOG_D(NR_MAC, "[Type-II] Phase packed: fallback=%d, orig_sb=%d, total wb_bits=%d\n",
                  use_fallback, num_sb, wb_bits);

            // Pack using standard CSI bitlen for compatibility with gNB zero-padding
            p1_bits = nr_get_csi_bitlen(csi_report);
            int padding_t2 = p1_bits - (cri_bitlen + ri_bitlen + pmi_x1_bitlen + cqi_bitlen);
            if (padding_t2 < 0) padding_t2 = 0;
            temp_payload_1 = 0;
            int shift = 0;
            temp_payload_1 |= ((uint64_t)mac->csirs_measurements.cqi & ((1ULL << cqi_bitlen) - 1));
            shift += cqi_bitlen;
            temp_payload_1 |= (wb_pmi << shift);
            shift += pmi_x1_bitlen;
            shift += padding_t2;
            temp_payload_1 |= ((uint64_t)mac->csirs_measurements.rank_indicator << shift);
            shift += ri_bitlen;
            p1_bits = shift;

            LOG_D(NR_MAC, "[Type-II] PMI+phase payload: 0x%lx (pmi=%d, pad=%d, total=%d bits)\n",
                  (unsigned long)temp_payload_1, wb_bits, padding_t2, p1_bits);
          } else {
          // Type-I original path
          int padding_bitlen = 0;
          if (mapping_type == ON_PUSCH) {
            p1_bits = cri_bitlen + ri_bitlen + cqi_bitlen;
            p2_bits = pmi_x1_bitlen + pmi_x2_bitlen;
            temp_payload_1 = (0 << (cqi_bitlen + ri_bitlen)) |
            ...
            temp_payload_1 = (0 << (cqi_bitlen + pmi_x2_bitlen + pmi_x1_bitlen + padding_bitlen + ri_bitlen)) |
