...
nfapi_nr_pm_list_t init_DL_MIMO_codebook(gNB_MAC_INST *gNB, nr_pdsch_AntennaPorts_t antenna_ports)
{
  ...
  // Allocate extra slots for dynamically reconstructed Type-II precoding matrices
  // Per-UE: TYPE2_PM_SLOTS_PER_UE slots per UE (SU: 2 slots + MU: 2 slots, separated to avoid overwrites)
  #define TYPE2_PM_RESERVED_SLOTS (MAX_MOBILES_PER_GNB * TYPE2_PM_SLOTS_PER_UE)
  mat.pmi_pdu = malloc16((mat.num_pm_idx + TYPE2_PM_RESERVED_SLOTS) * sizeof(*mat.pmi_pdu));
  AssertFatal(mat.pmi_pdu != NULL, "out of memory\n");
  memset(mat.pmi_pdu + mat.num_pm_idx, 0, TYPE2_PM_RESERVED_SLOTS * sizeof(*mat.pmi_pdu));
