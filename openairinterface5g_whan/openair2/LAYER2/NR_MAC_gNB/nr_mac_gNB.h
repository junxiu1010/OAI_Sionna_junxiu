...
#include "openair2/RRC/NR/nr_detailed_config.h"
...
typedef struct nr_type2_codebook_config {
  bool enabled;
  uint8_t num_beams;        // L: 2, 3, or 4
  uint8_t port_sel_d;       // d: 1, 2, 3, or 4
  uint8_t phase_alphabet;   // 4 (QPSK) or 8 (8PSK)
  bool subband_amplitude;
} nr_type2_codebook_config_t;
...
typedef struct nr_mac_config_t {
  ...
  bool do_SINR;
  nr_type2_codebook_config_t type2_codebook;
  csirs_detailed_config_t csirs_config;
  srs_detailed_config_t srs_config;
  csi_measurement_config_t csi_measurement_config;
} nr_mac_config_t;
...
typedef struct NR_sched_pdsch {
  ...
  // MU-MIMO fields
  bool mu_mimo_paired;
  bool mu_mimo_secondary;
  uint8_t mu_mimo_dmrs_port;
  uint8_t mu_mimo_group_size;
} NR_sched_pdsch_t;
...
#define TYPE2_MAX_BEAMS 4
#define TYPE2_MAX_LAYERS 2
#define TYPE2_MAX_SUBBANDS 19
...
struct CRI_RI_LI_PMI_CQI {
  ...
  bool is_type2;
  uint8_t port_selection_indicator;
  uint8_t strongest_coeff_indicator[TYPE2_MAX_LAYERS];
  uint8_t wideband_amplitude[2 * TYPE2_MAX_BEAMS][TYPE2_MAX_LAYERS];
  uint8_t subband_phase[TYPE2_MAX_SUBBANDS][2 * TYPE2_MAX_BEAMS][TYPE2_MAX_LAYERS];
  uint8_t subband_amp[TYPE2_MAX_SUBBANDS][2 * TYPE2_MAX_BEAMS][TYPE2_MAX_LAYERS];
...
typedef struct {
  ...
  int type2_pm_toggle;
  int type2_mu_pm_toggle;
} NR_UE_sched_ctrl_t;

#define TYPE2_PM_SU_SLOTS  2
#define TYPE2_PM_MU_SLOTS  2
#define TYPE2_PM_SLOTS_PER_UE (TYPE2_PM_SU_SLOTS + TYPE2_PM_MU_SLOTS)
