                                       ...
                                       int current_harq_pid)
{
  ...
  * this helps allocate retransmission when TDA changes (e.g. new nrOfSymbols > old nrOfSymbols) */
  ...
  sched_ctrl->sched_pdsch.mu_mimo_paired = false;
  sched_ctrl->sched_pdsch.mu_mimo_secondary = false;
  sched_ctrl->sched_pdsch.mu_mimo_dmrs_port = 0;
  sched_ctrl->sched_pdsch.mu_mimo_group_size = 1;
  /* retransmissions: directly allocate */
  *n_rb_sched -= sched_ctrl->sched_pdsch.rbSize;

  for (int rb = rbStart; rb < rbStart + sched_ctrl->sched_pdsch.rbSize; rb++)
    rballoc_mask[rb] |= SL_to_bitmap(sched_ctrl->sched_pdsch.tda_info.startSymbolIndex,
                                     sched_ctrl->sched_pdsch.tda_info.nrOfSymbols);
...
// ─── SUS + PF MU-MIMO User Selection ───

#define SUS_CORR_THRESHOLD_X1000 950  /* 0.95 — ZF/RZF precoding suppresses inter-user interference */
#define SUS_MAX_MU_USERS 4

static float pm_weight_correlation(const nfapi_nr_pm_pdu_t *pm1,
                                    const nfapi_nr_pm_pdu_t *pm2,
                                    int num_ap)
{
  int64_t dot_re = 0, dot_im = 0, norm1 = 0, norm2 = 0;
  for (int a = 0; a < num_ap && a < 4; a++) {
    int16_t w1r = pm1->weights[0][a].precoder_weight_Re;
    int16_t w1i = pm1->weights[0][a].precoder_weight_Im;
    int16_t w2r = pm2->weights[0][a].precoder_weight_Re;
    int16_t w2i = pm2->weights[0][a].precoder_weight_Im;
    dot_re += (int64_t)w1r * w2r + (int64_t)w1i * w2i;
    dot_im += (int64_t)w1r * w2i - (int64_t)w1i * w2r;
    norm1  += (int64_t)w1r * w1r + (int64_t)w1i * w1i;
    norm2  += (int64_t)w2r * w2r + (int64_t)w2i * w2i;
  }
  if (norm1 == 0 || norm2 == 0) return 1.0f;
  double dp = (double)dot_re * dot_re + (double)dot_im * dot_im;
  double np = (double)norm1 * norm2;
  return (float)(dp / np);
}

static float compute_pmi_correlation(const NR_UE_info_t *ue1,
                                     const NR_UE_info_t *ue2,
                                     const gNB_MAC_INST *mac)
{
  const struct CRI_RI_LI_PMI_CQI *r1 = &ue1->UE_sched_ctrl.CSI_report.cri_ri_li_pmi_cqi_report;
  const struct CRI_RI_LI_PMI_CQI *r2 = &ue2->UE_sched_ctrl.CSI_report.cri_ri_li_pmi_cqi_report;
  int num_ap = mac->radio_config.pdsch_AntennaPorts.N1
             * mac->radio_config.pdsch_AntennaPorts.N2
             * mac->radio_config.pdsch_AntennaPorts.XP;

  /* Path A: Full Type-II reconstruction from latest CSI report data */
  if (r1->is_type2 && r2->is_type2) {
    int report_id1 = r1->csi_report_id;
    int report_id2 = r2->csi_report_id;
    const nr_csi_report_t *cr1 = &ue1->csi_report_template[report_id1];
    const nr_csi_report_t *cr2 = &ue2->csi_report_template[report_id2];

    nfapi_nr_pm_pdu_t pm1, pm2;
    memset(&pm1, 0, sizeof(pm1));
    memset(&pm2, 0, sizeof(pm2));
    reconstruct_type2_precoding_matrix(r1, cr1, num_ap, 1, &pm1);
    reconstruct_type2_precoding_matrix(r2, cr2, num_ap, 1, &pm2);
    float corr = pm_weight_correlation(&pm1, &pm2, num_ap);
    LOG_D(NR_MAC, "[SUS-CORR-DBG] PathA(Type-II) ue1=%04x ue2=%04x corr=%.4f\n",
          ue1->rnti, ue2->rnti, corr);
    return corr;
  }

  /* Path B: Compare actual PM weights from the PM list (covers Type-I codebook
     entries, previously-reconstructed Type-II entries, and ZF-computed entries) */
  uint16_t pi1 = ue1->UE_sched_ctrl.sched_pdsch.pm_index;
  uint16_t pi2 = ue2->UE_sched_ctrl.sched_pdsch.pm_index;
  if (pi1 == 0 || pi2 == 0) return 1.0f;
  if (pi1 == pi2) return 1.0f;

  const nfapi_nr_pm_list_t *pml = &mac->config[0].pmi_list;
  float corr = pm_weight_correlation(&pml->pmi_pdu[pi1 - 1],
                                     &pml->pmi_pdu[pi2 - 1], num_ap);
  LOG_D(NR_MAC, "[SUS-CORR-DBG] PathB(PM-list) ue1=%04x(pm=%d,t2=%d) ue2=%04x(pm=%d,t2=%d) corr=%.4f\n",
        ue1->rnti, pi1, r1->is_type2, ue2->rnti, pi2, r2->is_type2, corr);
  return corr;
}
                                 ...
                                 NR_UE_info_t **selected)
{
  ...
  for (int i = 0; i < num_candidates && UE_sorted[i].UE != NULL; i++) {
    ...
    NR_UE_info_t *cand = UE_sorted[i].UE;
    ...
    for (int s = 0; s < n_sel; s++) {
      float c = compute_pmi_correlation(cand, selected[s], mac);
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    sched_pdsch->mu_mimo_paired = false;
    sched_pdsch->mu_mimo_secondary = false;
    sched_pdsch->mu_mimo_dmrs_port = 0;
    sched_pdsch->mu_mimo_group_size = 1;
    ...
    if (sched_pdsch->dl_harq_pid >= 0) {
      ...
      /* DTX-aware MCS floor: when PUCCH DTX rate is very high (>60%), the BLER
         estimate is unreliable (DTX counted as NACK). Use CQI as secondary
         indicator to prevent MCS from being stuck at 0. */
      {
        uint64_t total_tx = stats->rounds[0] + stats->rounds[1] + stats->rounds[2] + stats->rounds[3];
        if (total_tx > 100 && sched_pdsch->mcs == 0) {
          float dtx_rate = (float)UE->mac_stats.pucch0_DTX / (float)total_tx;
          int cqi = sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report.wb_cqi_1tb;
          if (dtx_rate > 0.6f && cqi >= 10) {
            int mcs_floor = (cqi >= 13) ? 4 : 2;
            if (mcs_floor > max_mcs) mcs_floor = max_mcs;
            sched_pdsch->mcs = mcs_floor;
            sched_ctrl->dl_bler_stats.mcs = mcs_floor;
            LOG_I(NR_MAC, "[DTX-MCS] UE %04x: DTX=%.0f%% CQI=%d → MCS floor=%d\n",
                  UE->rnti, dtx_rate * 100, cqi, mcs_floor);
          }
        }
      }
      sched_pdsch->nrOfLayers = get_dl_nrOfLayers(sched_ctrl, current_BWP->dci_format);
      if (sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report.is_type2) {
        const int report_id = sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report.csi_report_id;
        const nr_csi_report_t *csi_rpt = &UE->csi_report_template[report_id];
        int num_ap = mac->radio_config.pdsch_AntennaPorts.N1 * mac->radio_config.pdsch_AntennaPorts.N2
                     * mac->radio_config.pdsch_AntennaPorts.XP;
        nfapi_nr_pm_list_t *pm_list = &mac->config[CC_id].pmi_list;
        int ue_pm_base = pm_list->num_pm_idx + (UE->uid * TYPE2_PM_SLOTS_PER_UE);
        int ue_pm_toggle = sched_ctrl->type2_pm_toggle & 1;
        sched_ctrl->type2_pm_toggle++;
        int type2_pm_slot = ue_pm_base + ue_pm_toggle;
        nfapi_nr_pm_pdu_t *pm_entry = &pm_list->pmi_pdu[type2_pm_slot];
        memset(pm_entry, 0, sizeof(*pm_entry));
        reconstruct_type2_precoding_matrix(&sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report,
                                           csi_rpt, num_ap, sched_pdsch->nrOfLayers, pm_entry);
        pm_entry->pm_idx = type2_pm_slot + 1;
        sched_pdsch->pm_index = type2_pm_slot + 1;
        LOG_D(NR_MAC, "[Type-II] UE %04x uid=%d pm_slot=%d layers=%d\n",
              UE->rnti, UE->uid, type2_pm_slot, sched_pdsch->nrOfLayers);
      } else {
  ...
  int total_new_ues = 0;
  for (int i = 0; i < (int)sizeofArray(UE_sched) && UE_sched[i].UE != NULL; i++) total_new_ues++;

  if (mac->force_mu_dl_traffic) {
    int dbg_rem = 0;
    for (int i = 0; i < num_beams; i++) dbg_rem += remainUEs[i];
    LOG_I(NR_MAC, "[SUS-DBG] frame=%d slot=%d UE_sched=%d remainUEs=%d curUE=%d\n",
          frame, slot, total_new_ues, dbg_rem, curUE);
  }

  const int min_rbSize = 5;

  /* ===== SUS-FIRST: select orthogonal UE group from ENTIRE pool before resource allocation ===== */
  const int N_tx = mac->radio_config.pdsch_AntennaPorts.N1
                 * mac->radio_config.pdsch_AntennaPorts.N2
                 * mac->radio_config.pdsch_AntennaPorts.XP;
  bool sus_first_allocated[MAX_MOBILES_PER_GNB] = {false};
  int sus_first_count = 0;

  if (mac->mu_mimo_enabled && total_new_ues >= 2 && N_tx >= 2) {
    UEsched_t sus_pool[MAX_MOBILES_PER_GNB + 1];
    memset(sus_pool, 0, sizeof(sus_pool));
    int n_pool = 0;
    for (int i = 0; i < total_new_ues; i++) {
      NR_UE_info_t *ue_i = UE_sched[i].UE;
      NR_UE_sched_ctrl_t *sc_i = &ue_i->UE_sched_ctrl;
      if (sc_i->ul_failure) continue;
      if (sc_i->available_dl_harq.head < 0) continue;
      /* Skip UEs without valid directional info — MU-MIMO correlation
         and ZF precoding require a known channel direction */
      if (!sc_i->CSI_report.cri_ri_li_pmi_cqi_report.is_type2
          && sc_i->sched_pdsch.pm_index == 0) continue;
      sus_pool[n_pool].coef = UE_sched[i].coef;
      sus_pool[n_pool].UE   = ue_i;
      n_pool++;
    }
    ...
    if (n_pool >= 2) {
      ...
      if (n_sus >= 2) {
        ...
        for (int k = 0; k < n_sus; k++) {
          NR_UE_info_t *ue_k = sus_selected[k];
          NR_UE_sched_ctrl_t *sc_k = &ue_k->UE_sched_ctrl;
          NR_UE_DL_BWP_t *dl_bwp_k = &ue_k->current_DL_BWP;
          NR_UE_UL_BWP_t *ul_bwp_k = &ue_k->current_UL_BWP;
          NR_sched_pdsch_t *sp_k = &sc_k->sched_pdsch;

          NR_beam_alloc_t bm = beam_allocation_procedure(&mac->beam_info, frame, slot,
                                                         ue_k->UE_beam_index, slots_per_frame);
          if (bm.idx < 0) continue;
          if (remainUEs[bm.idx] == 0) {
            reset_beam_status(&mac->beam_info, frame, slot, ue_k->UE_beam_index, slots_per_frame, bm.new_beam);
            continue;
          }

          sp_k->dl_harq_pid = sc_k->available_dl_harq.head;
          sp_k->time_domain_allocation = get_dl_tda(mac, slot);
          if (sp_k->time_domain_allocation < 0) {
            reset_beam_status(&mac->beam_info, frame, slot, ue_k->UE_beam_index, slots_per_frame, bm.new_beam);
            continue;
          }

          const int csid = sc_k->coreset->controlResourceSetId;
          sp_k->tda_info = get_dl_tda_info(dl_bwp_k,
                                           sc_k->search_space->searchSpaceType->present,
                                           sp_k->time_domain_allocation,
                                           scc->dmrs_TypeA_Position, 1, TYPE_C_RNTI_, csid, false);
          if (!sp_k->tda_info.valid_tda) {
            reset_beam_status(&mac->beam_info, frame, slot, ue_k->UE_beam_index, slots_per_frame, bm.new_beam);
            continue;
          }

          int CCEIdx = get_cce_index(mac, CC_id, slot, ue_k->rnti,
                                     &sc_k->aggregation_level, bm.idx,
                                     sc_k->search_space, sc_k->coreset,
                                     &sc_k->sched_pdcch, false, sc_k->pdcch_cl_adjust);
          if (CCEIdx < 0) {
            reset_beam_status(&mac->beam_info, frame, slot, ue_k->UE_beam_index, slots_per_frame, bm.new_beam);
            continue;
          }
          ...
          int alloc_pucch = -1;
          if (!get_FeedbackDisabled(ue_k->sc_info.downlinkHARQ_FeedbackDisabled_r17, sp_k->dl_harq_pid)) {
            int r_pucch = nr_get_pucch_resource(sc_k->coreset, ul_bwp_k->pucch_Config, CCEIdx);
            alloc_pucch = nr_acknack_scheduling(mac, ue_k, frame, slot, ue_k->UE_beam_index, r_pucch, 0);
            if (alloc_pucch < 0) {
              reset_beam_status(&mac->beam_info, frame, slot, ue_k->UE_beam_index, slots_per_frame, bm.new_beam);
              continue;
            }
          ...
          sc_k->cce_index = CCEIdx;
          fill_pdcch_vrb_map(mac, CC_id, &sc_k->sched_pdcch, CCEIdx, sc_k->aggregation_level, bm.idx);
          sp_k->pucch_allocation = alloc_pucch;
          sus_beam_idx = bm.idx;
          confirmed[n_confirmed++] = ue_k;
          remainUEs[bm.idx]--;
        }

        if (n_confirmed >= 2) {
          NR_sched_pdsch_t *sp_lead = &confirmed[0]->UE_sched_ctrl.sched_pdsch;
          sp_lead->bwp_info = get_pdsch_bwp_start_size(mac, confirmed[0]);

          uint16_t *rballoc = mac->common_channels[CC_id].vrb_map[sus_beam_idx];
          const uint16_t slbm = SL_to_bitmap(sp_lead->tda_info.startSymbolIndex,
                                             sp_lead->tda_info.nrOfSymbols);
          int bwpStart = sp_lead->bwp_info.bwpStart;
          int bwpSize  = sp_lead->bwp_info.bwpSize;

          int rbStart_sus = 0;
          while (rbStart_sus < bwpSize && (rballoc[rbStart_sus + bwpStart] & slbm))
            rbStart_sus++;
          uint16_t maxRB_sus = 0;
          while (rbStart_sus + maxRB_sus < bwpSize && !(rballoc[rbStart_sus + maxRB_sus + bwpStart] & slbm))
            maxRB_sus++;

          if (maxRB_sus >= min_rbSize) {
            sp_lead->dmrs_parms = get_dl_dmrs_params(scc, &confirmed[0]->current_DL_BWP,
                                                     &sp_lead->tda_info, 1);
            sp_lead->Qm = nr_get_Qm_dl(sp_lead->mcs, confirmed[0]->current_DL_BWP.mcsTableIdx);
            sp_lead->R  = nr_get_code_rate_dl(sp_lead->mcs, confirmed[0]->current_DL_BWP.mcsTableIdx);
            uint32_t TBS_sus = 0;
            uint16_t rbSize_sus = 0;
            const int oh = 3 * 4 + 2;
            nr_find_nb_rb(sp_lead->Qm, sp_lead->R, 1, 1,
                          sp_lead->tda_info.nrOfSymbols,
                          sp_lead->dmrs_parms.N_PRB_DMRS * sp_lead->dmrs_parms.N_DMRS_SLOT,
                          confirmed[0]->UE_sched_ctrl.num_total_bytes + oh,
                          min_rbSize, maxRB_sus, &TBS_sus, &rbSize_sus);

            for (int rb = 0; rb < rbSize_sus; rb++)
            ...
            nfapi_nr_pm_pdu_t h_est[SUS_MAX_MU_USERS];
            nfapi_nr_pm_pdu_t zf_pm[SUS_MAX_MU_USERS];
            memset(h_est, 0, sizeof(h_est));
            int n_type2 = 0;
            for (int k = 0; k < n_confirmed; k++) {
              NR_UE_sched_ctrl_t *sc_k = &confirmed[k]->UE_sched_ctrl;
              if (sc_k->CSI_report.cri_ri_li_pmi_cqi_report.is_type2) {
                const int rid = sc_k->CSI_report.cri_ri_li_pmi_cqi_report.csi_report_id;
                reconstruct_type2_precoding_matrix(&sc_k->CSI_report.cri_ri_li_pmi_cqi_report,
                                                   &confirmed[k]->csi_report_template[rid],
                                                   N_tx, 1, &h_est[k]);
                n_type2++;
              }
            }

            /* === Phase 2: Compute joint ZF/RZF precoding if all UEs have Type-II PMI === */
            bool use_zf = (n_type2 == n_confirmed && n_confirmed >= 2);
            if (use_zf)
              compute_zf_precoding(h_est, n_confirmed, N_tx, zf_pm);

            /* === Phase 3: Assign precoding and scheduling parameters === */
            for (int k = 0; k < n_confirmed; k++) {
              NR_UE_info_t *ue_k = confirmed[k];
              NR_UE_sched_ctrl_t *sc_k = &ue_k->UE_sched_ctrl;
              NR_sched_pdsch_t *sp_k = &sc_k->sched_pdsch;

              sp_k->rbStart = rbStart_sus;
              sp_k->rbSize  = rbSize_sus;
              sp_k->tb_size = TBS_sus;
              sp_k->tda_info = sp_lead->tda_info;
              sp_k->time_domain_allocation = sp_lead->time_domain_allocation;
              sp_k->nrOfLayers = 1;
              sp_k->mu_mimo_paired = true;
              sp_k->mu_mimo_secondary = (k > 0);
              sp_k->mu_mimo_dmrs_port = (uint8_t)k;
              sp_k->mu_mimo_group_size = (uint8_t)n_confirmed;
              sp_k->bwp_info = sp_lead->bwp_info;

              sp_k->dmrs_parms = get_dl_dmrs_params(scc, &ue_k->current_DL_BWP, &sp_k->tda_info, 1);
              sp_k->Qm = nr_get_Qm_dl(sp_k->mcs, ue_k->current_DL_BWP.mcsTableIdx);
              sp_k->R  = nr_get_code_rate_dl(sp_k->mcs, ue_k->current_DL_BWP.mcsTableIdx);

              if (use_zf) {
                nfapi_nr_pm_list_t *pml = &mac->config[CC_id].pmi_list;
                int mu_base = pml->num_pm_idx + (ue_k->uid * TYPE2_PM_SLOTS_PER_UE) + TYPE2_PM_SU_SLOTS;
                int mu_tog = sc_k->type2_mu_pm_toggle & 1;
                sc_k->type2_mu_pm_toggle++;
                int mu_slot = mu_base + mu_tog;
                nfapi_nr_pm_pdu_t *pm_e = &pml->pmi_pdu[mu_slot];
                *pm_e = zf_pm[k];
                pm_e->pm_idx = mu_slot + 1;
                sp_k->pm_index = mu_slot + 1;
              } else if (sc_k->CSI_report.cri_ri_li_pmi_cqi_report.is_type2) {
                const int rid = sc_k->CSI_report.cri_ri_li_pmi_cqi_report.csi_report_id;
                const nr_csi_report_t *crpt = &ue_k->csi_report_template[rid];
                nfapi_nr_pm_list_t *pml = &mac->config[CC_id].pmi_list;
                int mu_base = pml->num_pm_idx + (ue_k->uid * TYPE2_PM_SLOTS_PER_UE) + TYPE2_PM_SU_SLOTS;
                int mu_tog = sc_k->type2_mu_pm_toggle & 1;
                sc_k->type2_mu_pm_toggle++;
                int mu_slot = mu_base + mu_tog;
                nfapi_nr_pm_pdu_t *pm_e = &pml->pmi_pdu[mu_slot];
                memset(pm_e, 0, sizeof(*pm_e));
                reconstruct_type2_precoding_matrix(&sc_k->CSI_report.cri_ri_li_pmi_cqi_report,
                                                   crpt, N_tx, 1, pm_e);
                pm_e->pm_idx = mu_slot + 1;
                sp_k->pm_index = mu_slot + 1;
              } else {
                NR_UE_DL_BWP_t *bwpk = &ue_k->current_DL_BWP;
                sp_k->pm_index = get_pm_index(mac, ue_k, bwpk->dci_format, 1,
                                              mac->radio_config.pdsch_AntennaPorts.XP);
              }

              for (int j = 0; j < total_new_ues; j++) {
                if (UE_sched[j].UE == ue_k) {
                  sus_first_allocated[j] = true;
                  break;
                }
              }
              sus_first_count++;
        ...
        LOG_I(NR_MAC, "[SUS-FIRST-REJECT] f=%d s=%d pool=%d n_sus=%d (corr_thr=%.3f)\n",
              frame, slot, n_pool, n_sus, (float)SUS_CORR_THRESHOLD_X1000 / 1000.0f);
      }
    }
  }
  int dbg_fail_harq = 0, dbg_fail_beam = 0, dbg_fail_remain = 0;
  int dbg_fail_rb = 0, dbg_fail_cce = 0, dbg_fail_pucch = 0, dbg_ok = 0;

  /* Loop UE_sched to find max coeff and allocate transmission */
  int iter_idx = 0;
  while (iterator->UE != NULL) {

    if (sus_first_allocated[iter_idx]) {
      iter_idx++;
      iterator++;
      continue;
    }
    ...
    if (CCEIndex < 0) {
      if (mac->force_mu_dl_traffic)
        LOG_I(NR_MAC, "[CCE-FAIL] UE %04x f=%d s=%d AL=%d coreset=%ld cl_adj=%.2f\n",
              rnti, frame, slot, sched_ctrl->aggregation_level,
              sched_ctrl->coreset->controlResourceSetId, sched_ctrl->pdcch_cl_adjust);
      reset_beam_status(&mac->beam_info, frame, slot, iterator->UE->UE_beam_index, slots_per_frame, beam.new_beam);
      dbg_fail_cce++;
      iter_idx++;
      iterator++;
  ...
  if (mac->force_mu_dl_traffic && curUE > 1) {
    LOG_I(NR_MAC, "[SUS-ALLOC] f=%d s=%d ok=%d harq=%d beam=%d remain=%d rb=%d cce=%d pucch=%d\n",
          frame, slot, dbg_ok, dbg_fail_harq, dbg_fail_beam, dbg_fail_remain,
          dbg_fail_rb, dbg_fail_cce, dbg_fail_pucch);
  }

  /* ===== SUS overlay (Phase 2 fallback for UEs not in SUS-first group) ===== */
  int num_eligible = 0;
  for (int i = 0; i < (int)sizeofArray(UE_sched) && UE_sched[i].UE != NULL; i++) {
    NR_sched_pdsch_t *sp_e = &UE_sched[i].UE->UE_sched_ctrl.sched_pdsch;
    if (sp_e->rbSize > 0 && !sp_e->mu_mimo_paired)
      num_eligible++;
  }

  if (mac->mu_mimo_enabled && num_eligible >= 2 && N_tx >= 2) {
    UEsched_t mu_cand[MAX_MOBILES_PER_GNB + 1];
    memset(mu_cand, 0, sizeof(mu_cand));
    int n_cand = 0;
    for (int i = 0; i < (int)sizeofArray(UE_sched) && UE_sched[i].UE != NULL; i++) {
      NR_UE_sched_ctrl_t *sc_c = &UE_sched[i].UE->UE_sched_ctrl;
      NR_sched_pdsch_t *sp_c = &sc_c->sched_pdsch;
      if (sp_c->rbSize <= 0) continue;
      if (sp_c->mu_mimo_paired) continue;
      if (sp_c->dl_harq_pid >= 0 && sc_c->harq_processes[sp_c->dl_harq_pid].round > 0) continue;
      if (!sc_c->CSI_report.cri_ri_li_pmi_cqi_report.is_type2
          && sp_c->pm_index == 0) continue;
      mu_cand[n_cand].coef = UE_sched[i].coef;
      mu_cand[n_cand].UE   = UE_sched[i].UE;
      n_cand++;
    }
    ...
    if (n_sel < 2 && n_cand >= 2) {
      LOG_I(NR_MAC, "[SUS-REJECT] f=%d s=%d n_cand=%d n_sel=%d (corr_thr=%.3f)\n",
            frame, slot, n_cand, n_sel, (float)SUS_CORR_THRESHOLD_X1000 / 1000.0f);
    }
    ...
    if (n_sel >= 2) {
      ...
      int n_p2 = 0;
      p2_confirmed[n_p2++] = selected[0]; /* leader always valid */
      for (int k = 1; k < n_sel; k++) {
        NR_sched_pdsch_t *sp_k = &selected[k]->UE_sched_ctrl.sched_pdsch;
        if (sp_lead->rbStart + sp_lead->rbSize > sp_k->bwp_info.bwpSize) {
          LOG_W(NR_MAC, "[MU-MIMO SUS] skip UE %04x: lead RB %d+%d exceeds BWP %d\n",
                selected[k]->rnti, sp_lead->rbStart, sp_lead->rbSize, sp_k->bwp_info.bwpSize);
          continue;
        }
        p2_confirmed[n_p2++] = selected[k];
      }

      if (n_p2 >= 2) {
        /* Pass 2: Reconstruct channel directions + compute ZF */
        nfapi_nr_pm_pdu_t p2_h_est[SUS_MAX_MU_USERS];
        nfapi_nr_pm_pdu_t p2_zf_pm[SUS_MAX_MU_USERS];
        memset(p2_h_est, 0, sizeof(p2_h_est));
        int p2_n_type2 = 0;
        for (int k = 0; k < n_p2; k++) {
          NR_UE_sched_ctrl_t *sc_k = &p2_confirmed[k]->UE_sched_ctrl;
          if (sc_k->CSI_report.cri_ri_li_pmi_cqi_report.is_type2) {
            const int rid = sc_k->CSI_report.cri_ri_li_pmi_cqi_report.csi_report_id;
            reconstruct_type2_precoding_matrix(&sc_k->CSI_report.cri_ri_li_pmi_cqi_report,
                                               &p2_confirmed[k]->csi_report_template[rid],
                                               N_tx, 1, &p2_h_est[k]);
            p2_n_type2++;
          }
        }
        bool p2_use_zf = (p2_n_type2 == n_p2 && n_p2 >= 2);
        if (p2_use_zf)
          compute_zf_precoding(p2_h_est, n_p2, N_tx, p2_zf_pm);

        /* Pass 3: Assign RBs, PM, and scheduling parameters */
        for (int k = 0; k < n_p2; k++) {
          NR_UE_info_t *ue_k = p2_confirmed[k];
          NR_UE_sched_ctrl_t *sc_k = &ue_k->UE_sched_ctrl;
          NR_sched_pdsch_t *sp_k = &sc_k->sched_pdsch;

          if (k > 0) {
            int bwp_start_k = sp_k->bwp_info.bwpStart;
            uint16_t slbitmap_k = SL_to_bitmap(sp_lead->tda_info.startSymbolIndex,
                                               sp_lead->tda_info.nrOfSymbols);
            int beam_idx_k = ue_k->UE_beam_index;
            if (beam_idx_k < 0) beam_idx_k = 0;
            uint16_t *rbmask_k = mac->common_channels[CC_id].vrb_map[beam_idx_k];
            for (int rb = 0; rb < sp_k->rbSize; rb++)
              rbmask_k[rb + sp_k->rbStart + bwp_start_k] &= ~slbitmap_k;
            n_rb_sched[beam_idx_k] += sp_k->rbSize;

            sp_k->rbStart = sp_lead->rbStart;
            sp_k->rbSize  = sp_lead->rbSize;
            sp_k->tda_info = sp_lead->tda_info;
            sp_k->dmrs_parms = sp_lead->dmrs_parms;
            sp_k->time_domain_allocation = sp_lead->time_domain_allocation;
            sp_k->tb_size = sp_lead->tb_size;
          }

          sp_k->mu_mimo_paired = true;
          sp_k->mu_mimo_secondary = (k > 0);
          sp_k->mu_mimo_dmrs_port = (uint8_t)k;
          sp_k->mu_mimo_group_size = (uint8_t)n_p2;
          sp_k->nrOfLayers = 1;
          ...
          if (p2_use_zf) {
            ...
            *pm_e = p2_zf_pm[k];
            ...
            sc_k->type2_mu_pm_toggle++;
            int mu_slot = mu_base + mu_tog;
            nfapi_nr_pm_pdu_t *pm_mu = &pml->pmi_pdu[mu_slot];
            memset(pm_mu, 0, sizeof(*pm_mu));
            reconstruct_type2_precoding_matrix(&sc_k->CSI_report.cri_ri_li_pmi_cqi_report,
                                               crpt, N_tx, 1, pm_mu);
            pm_mu->pm_idx = mu_slot + 1;
            sp_k->pm_index = mu_slot + 1;
          } else {
            NR_UE_DL_BWP_t *bwp_k = &ue_k->current_DL_BWP;
            sp_k->pm_index = get_pm_index(mac, ue_k, bwp_k->dci_format, 1,
                                          mac->radio_config.pdsch_AntennaPorts.XP);
          }
  ...
  /* ===== Sideband: log all scheduled DL UEs this slot ===== */
  UE_iterator(UE_list, UE_sb) {
    NR_UE_sched_ctrl_t *sc_sb = &UE_sb->UE_sched_ctrl;
    NR_sched_pdsch_t *sp_sb = &sc_sb->sched_pdsch;
    if (sp_sb->rbSize <= 0)
      continue;
    struct CRI_RI_LI_PMI_CQI *csi = &sc_sb->CSI_report.cri_ri_li_pmi_cqi_report;
    bool is_retx_sb = false;
    uint8_t harq_rnd = 0;
    if (sp_sb->dl_harq_pid >= 0) {
      NR_UE_harq_t *h = &sc_sb->harq_processes[sp_sb->dl_harq_pid];
      harq_rnd = h->round;
      is_retx_sb = (harq_rnd > 0);
    }
    uint16_t pair_rnti = 0;
    if (sp_sb->mu_mimo_paired) {
      UE_iterator(UE_list, UE_p) {
        NR_sched_pdsch_t *sp_p = &UE_p->UE_sched_ctrl.sched_pdsch;
        if (UE_p != UE_sb && sp_p->mu_mimo_paired && sp_p->rbSize > 0
            && sp_p->rbStart == sp_sb->rbStart && sp_p->rbSize == sp_sb->rbSize) {
          pair_rnti = UE_p->rnti;
          break;
        }
      }
    }
    mu_mimo_sched_entry_t entry = {
      .frame        = frame,
      .slot         = slot,
      .rnti         = UE_sb->rnti,
      .pm_index     = sp_sb->pm_index,
      .pmi_x1       = csi->pmi_x1,
      .pmi_x2       = csi->pmi_x2,
      .is_type2     = csi->is_type2,
      .mcs          = sp_sb->mcs,
      .rb_start     = sp_sb->rbStart,
      .rb_size      = sp_sb->rbSize,
      .n_layers     = sp_sb->nrOfLayers,
      .cqi          = csi->wb_cqi_1tb,
      .ri           = csi->ri,
      .is_mu_mimo   = sp_sb->mu_mimo_paired,
      .paired_rnti  = pair_rnti,
      .is_secondary = sp_sb->mu_mimo_secondary,
      .is_retx      = is_retx_sb,
      .harq_round   = harq_rnd,
      .tb_size      = sp_sb->tb_size,
    };
    mu_mimo_sideband_log_dl(&entry);
  }
}
                                                     ...
                                                     int pdu_index)
{
  ...
  if (sched_pdsch->mu_mimo_paired)
    pdsch_pdu->dmrsPorts = 1 << sched_pdsch->mu_mimo_dmrs_port;
  else
  ...
  if (final_pm_idx > 0) {
    nfapi_nr_pm_pdu_t *pm_check = &mac->config[0].pmi_list.pmi_pdu[final_pm_idx - 1];
    if (pm_check->numLayers != sched_pdsch->nrOfLayers) {
      LOG_W(NR_MAC, "[PM-MISMATCH] UE %04x pm_idx=%d nrOfLayers=%d pm_numLayers=%d -> fallback to identity\n",
            rnti, final_pm_idx, sched_pdsch->nrOfLayers, pm_check->numLayers);
      final_pm_idx = 0;
    }
  }
  pdsch_pdu->precodingAndBeamforming.prgs_list[0].pm_idx = final_pm_idx;
  pdsch_pdu->precodingAndBeamforming.prgs_list[0].dig_bf_interface_list[0].beam_idx = beam_index;

  if (sched_pdsch->mu_mimo_paired) {
    pdsch_pdu->mu_mimo_secondary = sched_pdsch->mu_mimo_secondary ? 1 : 0;
    int K = sched_pdsch->mu_mimo_group_size;
    if (K < 2) K = 2;
    static const int alpha_table[] = {32767, 32767, 23170, 18919, 16384};
    pdsch_pdu->mu_mimo_alpha_q15 = (K <= 4) ? alpha_table[K] : (int)(32767.0 / sqrt((double)K));
  } else {
    pdsch_pdu->mu_mimo_secondary = 0;
    pdsch_pdu->mu_mimo_alpha_q15 = 32767;
  }
                         ...
                         nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  UE_iterator(UE_info->connected_ue_list, UE) {
    ...
    if (sched_pdsch->mu_mimo_paired) {
      LOG_D(NR_MAC,
            "[MU-MIMO] %4d.%2d RNTI %04x %s PMI %d rb %d+%d MCS %d TBS %d alpha_q15 %d dmrsPorts 0x%x\n",
            frame, slot, rnti,
            sched_pdsch->mu_mimo_secondary ? "SECONDARY" : "PRIMARY",
            pdsch_pdu->precodingAndBeamforming.prgs_list[0].pm_idx,
            pdsch_pdu->rbStart, pdsch_pdu->rbSize,
            pdsch_pdu->mcsIndex[0], pdsch_pdu->TBSize[0],
            pdsch_pdu->mu_mimo_alpha_q15, pdsch_pdu->dmrsPorts);
    }
