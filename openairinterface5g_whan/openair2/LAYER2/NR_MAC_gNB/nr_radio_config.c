...
static void config_csirs(const NR_ServingCellConfigCommon_t *servingcellconfigcommon,
                         ...
                         int do_csirs,
                         int id,
                         const csirs_detailed_config_t *csirs_config)
{
  if (do_csirs) {
    ...
    *nzpid0 = id;
    ...
    if (csirs_config != NULL) {
      resourceMapping.firstOFDMSymbolInTimeDomain = csirs_config->first_symbol;
      resourceMapping.firstOFDMSymbolInTimeDomain2 = NULL;
      if (csirs_config->density == 3) {
      ...
      resourceMapping.freqBand.startingRB = csirs_config->freq_start_rb;
      resourceMapping.freqBand.nrofRBs = ((csirs_config->freq_nrof_rbs >> 2) + (csirs_config->freq_nrof_rbs % 4 > 0)) << 2;
      nzpcsi0->powerControlOffset = csirs_config->power_offset;
    } else {
    ...
    *nzpcsi0->powerControlOffsetSS = NR_NZP_CSI_RS_Resource__powerControlOffsetSS_db0;
    ...
    // 주기성 설정
    if (csirs_config != NULL) {
      // Configuration 기반 주기성 설정
      const frame_structure_t *fs = &(RC.nrmac[0]->frame_structure);
      set_csirs_periodicity(nzpcsi0, id, csirs_config->periodicity, fs);
    } else {
      // 기존 하드코딩된 주기성 설정
    ...
    if (csirs_config != NULL) {
      *nzpcsi0->qcl_InfoPeriodicCSI_RS = csirs_config->qcl_info;
    } else {
...
static NR_SRS_Resource_t *get_srs_resource(const NR_ServingCellConfigCommon_t *scc,
                                           ...
                                           int do_srs,
                                           const srs_detailed_config_t *srs_cfg)
{
  ...
  int comb_offset = (srs_cfg != NULL) ? srs_cfg->comb_offset : 0;
  int cyclic_shift = (srs_cfg != NULL) ? srs_cfg->cyclic_shift : 0;
  switch (tx_comb) {
    case NR_SRS_Resource__transmissionComb_PR_n2:
      ...
      srs_res->transmissionComb.choice.n2->cyclicShift_n2 = cyclic_shift;
      break;
    case NR_SRS_Resource__transmissionComb_PR_n4:
      ...
      srs_res->transmissionComb.choice.n4->cyclicShift_n4 = cyclic_shift;
      break;
    default:
      AssertFatal(1 == 0, "Invalid transmission comb %d\n", tx_comb);
  }
  if (srs_cfg != NULL) {
    srs_res->resourceMapping.startPosition = srs_cfg->start_position;
    switch (srs_cfg->num_symbols) {
      case 2:  srs_res->resourceMapping.nrofSymbols = NR_SRS_Resource__resourceMapping__nrofSymbols_n2; break;
      case 4:  srs_res->resourceMapping.nrofSymbols = NR_SRS_Resource__resourceMapping__nrofSymbols_n4; break;
      default: srs_res->resourceMapping.nrofSymbols = NR_SRS_Resource__resourceMapping__nrofSymbols_n1; break;
    }
    switch (srs_cfg->repetition_factor) {
      case 2:  srs_res->resourceMapping.repetitionFactor = NR_SRS_Resource__resourceMapping__repetitionFactor_n2; break;
      case 4:  srs_res->resourceMapping.repetitionFactor = NR_SRS_Resource__resourceMapping__repetitionFactor_n4; break;
      default: srs_res->resourceMapping.repetitionFactor = NR_SRS_Resource__resourceMapping__repetitionFactor_n1; break;
    }
    srs_res->freqDomainPosition = srs_cfg->freq_domain_position;
    srs_res->freqDomainShift = srs_cfg->freq_domain_shift;
    srs_res->freqHopping.b_SRS = srs_cfg->freq_hopping_b_srs;
    srs_res->freqHopping.b_hop = srs_cfg->freq_hopping_b_hop;
    srs_res->freqHopping.c_SRS = srs_cfg->freq_hopping_c_srs > 0
                                   ? srs_cfg->freq_hopping_c_srs
                                   : rrc_get_max_nr_csrs(curr_bwp, srs_res->freqHopping.b_SRS);
    if (strcmp(srs_cfg->group_hopping, "groupHopping") == 0)
      srs_res->groupOrSequenceHopping = NR_SRS_Resource__groupOrSequenceHopping_groupHopping;
    else if (strcmp(srs_cfg->group_hopping, "sequenceHopping") == 0)
      srs_res->groupOrSequenceHopping = NR_SRS_Resource__groupOrSequenceHopping_sequenceHopping;
    else
                                                    ...
                                                    const srs_detailed_config_t *srs_cfg)
{
  ...
  NR_SRS_Resource_t *srs_res0 =
      get_srs_resource(scc, uecap, curr_bwp, uid, res_id, maxMIMO_Layers, NR_SRS_Resource__transmissionComb_PR_n2, do_srs, srs_cfg);
...
static void config_csi_codebook(const nr_pdsch_AntennaPorts_t *antennaports,
                                ...
                                const nr_type2_codebook_config_t *type2_cfg)
{
  const int num_ant_ports = antennaports->N1 * antennaports->N2 * antennaports->XP;

  if (type2_cfg && type2_cfg->enabled) {
    codebookConfig->codebookType.present = NR_CodebookConfig__codebookType_PR_type2;
    if (!codebookConfig->codebookType.choice.type2)
      codebookConfig->codebookType.choice.type2 = calloc(1, sizeof(*codebookConfig->codebookType.choice.type2));
    struct NR_CodebookConfig__codebookType__type2 *type2 = codebookConfig->codebookType.choice.type2;

    type2->subType.present = NR_CodebookConfig__codebookType__type2__subType_PR_typeII_PortSelection;
    if (!type2->subType.choice.typeII_PortSelection)
      type2->subType.choice.typeII_PortSelection = calloc(1, sizeof(*type2->subType.choice.typeII_PortSelection));

    struct NR_CodebookConfig__codebookType__type2__subType__typeII_PortSelection *ps =
        type2->subType.choice.typeII_PortSelection;

    ps->portSelectionSamplingSize = calloc(1, sizeof(*ps->portSelectionSamplingSize));
    switch (type2_cfg->port_sel_d) {
      case 1: *ps->portSelectionSamplingSize = NR_CodebookConfig__codebookType__type2__subType__typeII_PortSelection__portSelectionSamplingSize_n1; break;
      case 2: *ps->portSelectionSamplingSize = NR_CodebookConfig__codebookType__type2__subType__typeII_PortSelection__portSelectionSamplingSize_n2; break;
      case 3: *ps->portSelectionSamplingSize = NR_CodebookConfig__codebookType__type2__subType__typeII_PortSelection__portSelectionSamplingSize_n3; break;
      case 4: *ps->portSelectionSamplingSize = NR_CodebookConfig__codebookType__type2__subType__typeII_PortSelection__portSelectionSamplingSize_n4; break;
      default: AssertFatal(false, "Invalid port selection sampling size d=%d\n", type2_cfg->port_sel_d);
    }

    int ri_restriction_val = (max_layers >= 2) ? 0x3 : 0x1;
    ps->typeII_PortSelectionRI_Restriction.size = 1;
    ps->typeII_PortSelectionRI_Restriction.bits_unused = 6;
    ps->typeII_PortSelectionRI_Restriction.buf = calloc(1, sizeof(uint8_t));
    ps->typeII_PortSelectionRI_Restriction.buf[0] = ri_restriction_val << 6;

    type2->phaseAlphabetSize = (type2_cfg->phase_alphabet == 8)
        ? NR_CodebookConfig__codebookType__type2__phaseAlphabetSize_n8
        : NR_CodebookConfig__codebookType__type2__phaseAlphabetSize_n4;
    type2->subbandAmplitude = type2_cfg->subband_amplitude ? true : false;
    switch (type2_cfg->num_beams) {
      case 2: type2->numberOfBeams = NR_CodebookConfig__codebookType__type2__numberOfBeams_two; break;
      case 3: type2->numberOfBeams = NR_CodebookConfig__codebookType__type2__numberOfBeams_three; break;
      case 4: type2->numberOfBeams = NR_CodebookConfig__codebookType__type2__numberOfBeams_four; break;
      default: AssertFatal(false, "Invalid number of beams L=%d\n", type2_cfg->num_beams);
    }

    LOG_I(NR_RRC, "[Type-II PortSel] Configured: d=%d, L=%d, phaseAlphabet=%d, subbandAmp=%d, ports=%d\n",
          type2_cfg->port_sel_d, type2_cfg->num_beams, type2_cfg->phase_alphabet,
          type2_cfg->subband_amplitude, num_ant_ports);
    return;
  }
                                   ...
                                   const nr_type2_codebook_config_t *type2_cfg)
{
  ...
  *csirep->csi_IM_ResourcesForInterference = im_id;
  ...
  // 상세 설정에 따른 보고 형식 설정 - 모든 측정 활성화
  csirep->reportQuantity.present = NR_CSI_ReportConfig__reportQuantity_PR_cri_RI_LI_PMI_CQI;
  csirep->reportQuantity.choice.cri_RI_LI_PMI_CQI = (NULL_t)0;
  
  // 디버그 로그 추가
  LOG_I(NR_RRC, "CSI Report Config: reportQuantity=cri_RI_LI_PMI_CQI (all measurements enabled), reportConfigId=%ld\n", csirep->reportConfigId);
  ...
  *csirep->reportFreqConfiguration->pmi_FormatIndicator = NR_CSI_ReportConfig__reportFreqConfiguration__pmi_FormatIndicator_widebandPMI;
  ...
  config_csi_codebook(antennaports, max_layers, csirep->codebookConfig, type2_cfg);
  csirep->dummy = NULL;
                                                   ...
                                                   const nr_mac_config_t *configuration)
{
  ...
  initialUplinkBWP->srs_Config = get_config_srs(scc, NULL, curr_bwp, uid, 0, maxMIMO_Layers, configuration->minRXTXTIME, 0, &configuration->srs_config);
  ...
  for (int bwp_loop = 0; bwp_loop < bwp_loop_end; bwp_loop++) {
    ...
    config_csirs(scc, csi_MeasConfig, pdsch_AntennaPorts, curr_bwp, configuration->do_CSIRS, bwp_loop, &configuration->csirs_config);
                            ...
                            const NR_ServingCellConfigCommon_t *scc)
{
  ...
  for (int report = 0; report < csi_MeasConfig->csi_ReportConfigToAddModList->list.count; report++) {
    ...
    if(csirep->codebookConfig)
      config_csi_codebook(&configuration->pdsch_AntennaPorts, *pdsch_servingcellconfig->ext1->maxMIMO_Layers, csirep->codebookConfig, &configuration->type2_codebook);
  }
                                                     ...
                                                     int uid)
{
  ...
  *secondaryCellGroup->spCellConfig->servCellIndex = servCellIndex;
  ...
  for (int bwp_loop = 0; bwp_loop < n_dl_bwp; bwp_loop++) {
    ...
    config_csirs(servingcellconfigcommon, csi_MeasConfig, dl_antenna_ports, curr_bwp, do_csirs, bwp_loop, &configuration->csirs_config);
