                           ...
                           slot_t slot)
{
  ...
  mu_mimo_harq_entry_t harq_entry = {
    .frame      = frame,
    .slot       = slot,
    .rnti       = UE->rnti,
    .harq_pid   = harq_pid,
    .ack        = success,
    .harq_round = harq->round,
  };
  mu_mimo_sideband_log_harq(&harq_entry);
  ...
  for (int i = 0; i < 8; i++) {
     if ((ri_restriction >> i) & 0x01) {
       if(count == ri_index) {
         ...
         LOG_I(NR_MAC, "[gNB] Rank Indicator (RI) Report: %d (bitlen: %d, restriction: 0x%02x)\n", 
               i + 1, ri_bitlen, ri_restriction);
                                ...
                                uint8_t cqi_Table)
{
  ...
  LOG_I(NR_MAC, "[gNB] CQI Report Details:\n");
  LOG_I(NR_MAC, "  - CQI Table: %d\n", cqi_Table);
  LOG_I(NR_MAC, "  - Wide-band CQI for TB1: %d (bitlen: %d)\n", temp_cqi, cqi_bitlen);
  ...
  if (cqi_bitlen > 4) {
    ...
    LOG_I(NR_MAC, "  - Wide-band CQI for TB2: %d\n", temp_cqi);
                                   ...
                                   NR_UE_sched_ctrl_t *sched_ctrl)
{
  ...
  if (csi_report->is_type2_port_selection) {
    struct CRI_RI_LI_PMI_CQI *rpt = &sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report;
    rpt->is_type2 = true;
    int starting_bit = cumul_bits;
    int num_layers = (ri >= 0) ? ri + 1 : 1;
    if (num_layers > TYPE2_MAX_LAYERS) num_layers = TYPE2_MAX_LAYERS;
    ...
    if (ps_bits > 0) {
      rpt->port_selection_indicator = pickandreverse_bits(payload, ps_bits, starting_bit);
      starting_bit += ps_bits;
    }
    ...
    for (int lay = 0; lay < num_layers; lay++) {
      rpt->strongest_coeff_indicator[lay] = pickandreverse_bits(payload, sc_bits, starting_bit);
      starting_bit += sc_bits;
    }

    // WB amplitudes (3 bits each, strongest omitted)
    int total_coeffs = 2 * L;
    if (total_coeffs > 2 * d) total_coeffs = 2 * d;
    for (int lay = 0; lay < num_layers; lay++) {
      for (int c = 0; c < total_coeffs; c++) {
        if (c == rpt->strongest_coeff_indicator[lay]) {
          rpt->wideband_amplitude[c][lay] = 0;
          continue;
        }
        rpt->wideband_amplitude[c][lay] = pickandreverse_bits(payload, 3, starting_bit);
        starting_bit += 3;
      }
    }

    // 1-bit flag: averaged subband phase present
    ...
    int total_parsed = starting_bit - cumul_bits;
    LOG_I(NR_MAC, "[Type-II PortSel] PMI parsed: port_sel=%d, L=%d, d=%d, layers=%d, has_phase=%d, bits=%d\n",
          rpt->port_selection_indicator, L, d, num_layers, has_phase, total_parsed);
    if (has_phase) {
      LOG_I(NR_MAC, "[Type-II PortSel] Avg phases: c0=%d c1=%d c2=%d c3=%d\n",
  ...
  sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report.is_type2 = false;
...
static NR_UE_harq_t *find_harq(frame_t frame, slot_t slot, NR_UE_info_t * UE, int harq_round_max)
{
  ...
  while ((harq->feedback_frame - frame + 1024 ) % 1024 > 512 // harq->feedback_frame < frame, distance of 512 is boundary to decide if feedback_frame is in the past or future
    ...
    handle_dl_harq(UE, pid, 0, harq_round_max, frame, slot);
...
void handle_nr_uci_pucch_0_1(module_id_t mod_id, frame_t frame, slot_t slot, const nfapi_nr_uci_pucch_pdu_format_0_1_t *uci_01)
{
  ...
  if (((uci_01->pduBitmap >> 1) & 0x01)) {
    ...
    for (int harq_bit = 0; harq_bit < uci_01->harq.num_harq; harq_bit++) {
      ...
      handle_dl_harq(UE, pid, success, nrmac->dl_bler.harq_round_max, frame, slot);
      if (is_ra) {
  ...
  if (uci_01->pduBitmap & 0x1) {
    if (uci_01->sr.sr_indication && uci_01->sr.sr_confidence_level == 0) {
      if (uci_01->ul_cqi >= 80) {
        ...
        LOG_W(NR_MAC, "SR UE %04x rejected: ul_cqi %d < 80\n", uci_01->rnti, uci_01->ul_cqi);
      }
    }
...
void handle_nr_uci_pucch_2_3_4(module_id_t mod_id, frame_t frame, slot_t slot, const nfapi_nr_uci_pucch_pdu_format_2_3_4_t *uci_234)
{
  ...
  if ((uci_234->pduBitmap >> 1) & 0x01) {
    ...
    for (int harq_bit = 0; harq_bit < uci_234->harq.harq_bit_len; harq_bit++) {
      ...
      handle_dl_harq(UE, pid, uci_234->harq.harq_crc != 1 && acknack, nrmac->dl_bler.harq_round_max, frame, slot);
    }
