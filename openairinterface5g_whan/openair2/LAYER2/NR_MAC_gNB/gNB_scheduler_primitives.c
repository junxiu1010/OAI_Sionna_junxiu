...
// Type-II Port Selection WB amplitude dequantization table (TS 38.214 Table 5.2.2.2.3-3)
static const double type2_wb_amp_table[8] = {
  1.0,          // index 0: 1
  0.7071,       // index 1: 1/sqrt(2)
  0.5,          // index 2: 1/2
  0.3536,       // index 3: 1/(2*sqrt(2))
  0.25,         // index 4: 1/4
  0.1768,       // index 5: 1/(4*sqrt(2))
  0.125,        // index 6: 1/8
  0.0           // index 7: 0
};

// QPSK/8PSK phase dequantization
static const double type2_qpsk_phase[4] = {0.0, M_PI / 2, M_PI, 3 * M_PI / 2};
static const double type2_8psk_phase[8] = {0.0, M_PI / 4, M_PI / 2, 3 * M_PI / 4,
                                           M_PI, 5 * M_PI / 4, 3 * M_PI / 2, 7 * M_PI / 4};

void reconstruct_type2_precoding_matrix(const struct CRI_RI_LI_PMI_CQI *rpt,
                                        const nr_csi_report_t *csi_report,
                                        int num_ant_ports,
                                        int num_layers,
                                        nfapi_nr_pm_pdu_t *pm_out)
{
  int L = csi_report->type2_num_beams;
  int d = csi_report->type2_port_sel_d;
  int N_phase = csi_report->type2_phase_alphabet;
  int sel_start = rpt->port_selection_indicator * d;

  pm_out->numLayers = num_layers;
  pm_out->num_ant_ports = num_ant_ports;
  memset(pm_out->weights, 0, sizeof(pm_out->weights));
...
/* ===== Regularized Zero-Forcing (RZF) MU-MIMO Precoding from Type-II PMI ===== */

#define ZF_MAX_UE  4
#define ZF_MAX_ANT 4
#define ZF_ALPHA   0.01  /* RZF regularization: trades interference nulling for noise robustness */

typedef struct { double re, im; } zf_cplx_t;

static inline zf_cplx_t zf_mul(zf_cplx_t a, zf_cplx_t b)
{
  return (zf_cplx_t){a.re * b.re - a.im * b.im, a.re * b.im + a.im * b.re};
}

static inline zf_cplx_t zf_conj(zf_cplx_t a)
{
  return (zf_cplx_t){a.re, -a.im};
}

static inline double zf_abs2(zf_cplx_t a)
{
  return a.re * a.re + a.im * a.im;
}

/* Gauss-Jordan inversion of K×K complex matrix in-place.
   aug = [G | I] is K × 2K.  On success G side becomes G^{-1}. Returns 0 on success. */
static int zf_invert(zf_cplx_t aug[][2 * ZF_MAX_UE], int K)
{
  for (int col = 0; col < K; col++) {
    /* partial pivoting */
    int piv = col;
    double best = zf_abs2(aug[col][col]);
    for (int r = col + 1; r < K; r++) {
      double v = zf_abs2(aug[r][col]);
      if (v > best) { best = v; piv = r; }
    }
    if (best < 1e-30) return -1;
    if (piv != col) {
      for (int c = 0; c < 2 * K; c++) {
        zf_cplx_t tmp = aug[col][c]; aug[col][c] = aug[piv][c]; aug[piv][c] = tmp;
      }
    }
    /* scale pivot row */
    zf_cplx_t d = aug[col][col];
    double dinv_den = zf_abs2(d);
    zf_cplx_t dinv = {d.re / dinv_den, -d.im / dinv_den};
    for (int c = 0; c < 2 * K; c++)
      aug[col][c] = zf_mul(aug[col][c], dinv);
    /* eliminate other rows */
    for (int r = 0; r < K; r++) {
      if (r == col) continue;
      zf_cplx_t f = aug[r][col];
      for (int c = 0; c < 2 * K; c++) {
        aug[r][c].re -= f.re * aug[col][c].re - f.im * aug[col][c].im;
        aug[r][c].im -= f.re * aug[col][c].im + f.im * aug[col][c].re;
      }
    }
  }
  return 0;
...
void compute_zf_precoding(const nfapi_nr_pm_pdu_t *h_est,
                          int K, int Nt,
                          nfapi_nr_pm_pdu_t *zf_out)
{
  AssertFatal(K <= ZF_MAX_UE && Nt <= ZF_MAX_ANT, "ZF: K=%d Nt=%d exceeds limits\n", K, Nt);

  /* Step 1: Q15 → double complex channel direction matrix H[K][Nt] */
  zf_cplx_t H[ZF_MAX_UE][ZF_MAX_ANT];
  memset(H, 0, sizeof(H));
  for (int k = 0; k < K; k++)
    for (int a = 0; a < Nt; a++)
      H[k][a] = (zf_cplx_t){h_est[k].weights[0][a].precoder_weight_Re / 32767.0,
                             h_est[k].weights[0][a].precoder_weight_Im / 32767.0};

  /* Step 2: Gram matrix G = H·H^H + αI  (K×K) */
  zf_cplx_t aug[ZF_MAX_UE][2 * ZF_MAX_UE];
  memset(aug, 0, sizeof(aug));
  for (int i = 0; i < K; i++) {
    for (int j = 0; j < K; j++) {
      zf_cplx_t s = {0, 0};
      for (int a = 0; a < Nt; a++) {
        zf_cplx_t hj_conj = zf_conj(H[j][a]);
        s.re += H[i][a].re * hj_conj.re - H[i][a].im * hj_conj.im;
        s.im += H[i][a].re * hj_conj.im + H[i][a].im * hj_conj.re;
      }
      aug[i][j] = s;
    }
    aug[i][i].re += ZF_ALPHA;  /* regularization */
    /* identity on the right half */
    aug[i][K + i] = (zf_cplx_t){1.0, 0.0};
  }

  /* Step 3: Invert G → G_inv stored in aug[i][K+j] */
  if (zf_invert(aug, K) != 0) {
    LOG_W(NR_MAC, "[ZF] Gram matrix singular, falling back to individual PMI\n");
    for (int k = 0; k < K; k++)
      zf_out[k] = h_est[k];
    return;
  }

  /* Step 4: P = H^H · G_inv  (Nt×K), then normalize columns
     P[a][k] = Σ_j  conj(H[j][a]) · G_inv[j][k] */
  zf_cplx_t P[ZF_MAX_ANT][ZF_MAX_UE];
  memset(P, 0, sizeof(P));
  for (int a = 0; a < Nt; a++) {
    for (int k = 0; k < K; k++) {
      zf_cplx_t s = {0, 0};
      for (int j = 0; j < K; j++) {
        zf_cplx_t hj_conj = zf_conj(H[j][a]);
        zf_cplx_t ginv_jk = aug[j][K + k];
        s.re += hj_conj.re * ginv_jk.re - hj_conj.im * ginv_jk.im;
        s.im += hj_conj.re * ginv_jk.im + hj_conj.im * ginv_jk.re;
      }
      P[a][k] = s;
    }
  }
  ...
  for (int k = 0; k < K; k++) {
    ...
    memset(&zf_out[k], 0, sizeof(zf_out[k]));
    zf_out[k].numLayers = 1;
    zf_out[k].num_ant_ports = Nt;
    for (int a = 0; a < Nt; a++) {
      zf_out[k].weights[0][a].precoder_weight_Re = (int16_t)(P[a][k].re * scale * 32767.0);
      zf_out[k].weights[0][a].precoder_weight_Im = (int16_t)(P[a][k].im * scale * 32767.0);
    }
  }

  LOG_I(NR_MAC, "[ZF] Computed RZF precoding for %d UEs, %d antennas (alpha=%.4f)\n",
        K, Nt, ZF_ALPHA);
}
...
{
  const float max_cl_adjust = 0.25f;
  if (feedback_not_detected) {
    sched_ctrl->pdcch_cl_adjust = min(max_cl_adjust, sched_ctrl->pdcch_cl_adjust + 0.05);
  } else {
