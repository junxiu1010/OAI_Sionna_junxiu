...
typedef struct{
  ...
  // Type-II Port Selection bit lengths
  bool is_type2_port_selection;
  uint16_t type2_wideband_pmi_bitlen[2];  // per rank (0=rank1, 1=rank2)
  uint16_t type2_subband_pmi_bitlen[2];
} CSI_Meas_bitlen_t;
...
//  uint8_t nb_of_nzp_csi_report;
  ...
  uint8_t type2_phase_alphabet;  // 4 (QPSK) or 8 (8PSK)
  bool type2_subband_amplitude;
  uint8_t type2_num_subbands;
} nr_csi_report_t;
