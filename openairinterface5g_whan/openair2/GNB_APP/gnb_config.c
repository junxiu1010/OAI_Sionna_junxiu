...
#include "gnb_detailed_params.h"
#include "lib/f1ap_interface_management.h"
#include "RRC/NR/nr_detailed_config.h"
...
static codebook_detailed_config_t read_codebook_detailed_config(configmodule_interface_t *cfg, int gnb_idx);
...
void RCconfig_nr_macrlc(configmodule_interface_t *cfg)
{
  ...
  // Type-II Port Selection codebook configuration
  {
    codebook_detailed_config_t cb_cfg = read_codebook_detailed_config(cfg, 0);
    if (strcmp(cb_cfg.codebook_type, "type2") == 0
        && strcmp(cb_cfg.sub_type, "typeII_PortSelection") == 0) {
      config.type2_codebook.enabled = true;
      config.type2_codebook.num_beams = cb_cfg.number_of_beams;
      config.type2_codebook.port_sel_d = cb_cfg.port_selection_sampling_size;
      config.type2_codebook.phase_alphabet = (strcmp(cb_cfg.phase_alphabet, "n8") == 0) ? 8 : 4;
      config.type2_codebook.subband_amplitude = cb_cfg.subband_amplitude ? true : false;
      LOG_I(GNB_APP, "Type-II Port Selection codebook enabled: L=%d, d=%d, phase=%d, sb_amp=%d\n",
            config.type2_codebook.num_beams, config.type2_codebook.port_sel_d,
            config.type2_codebook.phase_alphabet, config.type2_codebook.subband_amplitude);
    } else {
      config.type2_codebook.enabled = false;
      LOG_I(GNB_APP, "Using Type-I codebook (type=%s, sub_type=%s)\n",
            cb_cfg.codebook_type, cb_cfg.sub_type);
    }
  }

  config.csirs_config = read_csirs_detailed_config(cfg, 0);
  LOG_I(GNB_APP, "CSI-RS detailed config: periodicity=%d, nrof_ports=%d, first_symbol=%d, density=%d\n",
        config.csirs_config.periodicity, config.csirs_config.nrof_ports,
        config.csirs_config.first_symbol, config.csirs_config.density);

  config.srs_config = read_srs_detailed_config(cfg, 0);
  LOG_I(GNB_APP, "SRS detailed config: periodicity=%d, nrof_srs_ports=%d, p0=%d\n",
        config.srs_config.periodicity, config.srs_config.nrof_srs_ports, config.srs_config.p0);

  config.csi_measurement_config = read_csi_measurement_config(cfg, 0);
  LOG_I(GNB_APP, "CSI measurement config: report_periodicity=%d, cqi_table=%s\n",
        config.csi_measurement_config.report_periodicity, config.csi_measurement_config.cqi_table);
  ...
  if (MacRLC_ParamList.numelt > 0) {
    ...
    for (j = 0; j < RC.nb_nr_macrlc_inst; j++) {
      ...
      RC.nrmac[j]->force_mu_dl_traffic = *(MacRLC_ParamList.paramarray[j][MACRLC_FORCE_MU_DL_TRAFFIC_IDX].u8ptr);
      if (RC.nrmac[j]->mu_mimo_enabled)
        LOG_I(NR_MAC, "MU-MIMO downlink scheduling enabled\n");
      if (RC.nrmac[j]->force_mu_dl_traffic)
        LOG_I(NR_MAC, "MU-MIMO forced DL traffic injection enabled\n");
      // PRB Blacklist
      uint16_t prbbl[MAX_BWP_SIZE] = {0};
      int num_ulprbbl = get_prb_blacklist(j, prbbl);
      if (num_ulprbbl != -1) {
...
// ============================================================================
// CSI-RS/SRS/PMI 상세 설정 읽기 함수들
// ============================================================================

/**
 * @brief CSI-RS 상세 설정을 configuration에서 읽어옵니다
 * @param cfg Configuration 모듈 인터페이스
 * @param gnb_idx gNB 인덱스
 * @return CSI-RS 상세 설정 구조체
 */
 csirs_detailed_config_t read_csirs_detailed_config(configmodule_interface_t *cfg, int gnb_idx)
 {
   csirs_detailed_config_t csirs_config = {0};
   char aprefix[MAX_OPTNAME_SIZE * 2 + 8];
   
   snprintf(aprefix, sizeof(aprefix), "%s.[%i]", GNB_CONFIG_STRING_GNB_LIST, gnb_idx);
   
   // CSI-RS 기본 설정 읽기
   GET_PARAMS_LIST(CSIRSParamList, CSIRSParams, CSIRSPARAMS_DESC, GNB_CONFIG_STRING_CSIRS_DETAILED_CONFIG, aprefix);
   
   if (CSIRSParamList.numelt > 0) {
     csirs_config.periodicity = *CSIRSParamList.paramarray[0][0].uptr;
     csirs_config.first_symbol = *CSIRSParamList.paramarray[0][1].uptr;
     csirs_config.power_offset = *CSIRSParamList.paramarray[0][2].uptr;
     csirs_config.density = *CSIRSParamList.paramarray[0][3].uptr;
     csirs_config.freq_start_rb = *CSIRSParamList.paramarray[0][4].uptr;
     csirs_config.freq_nrof_rbs = *CSIRSParamList.paramarray[0][5].uptr;
     csirs_config.freq_allocation[0] = *CSIRSParamList.paramarray[0][6].uptr;
     csirs_config.freq_allocation[1] = *CSIRSParamList.paramarray[0][7].uptr;
     strcpy(csirs_config.cdm_type, *CSIRSParamList.paramarray[0][8].strptr);
     csirs_config.nrof_ports = *CSIRSParamList.paramarray[0][9].uptr;
     csirs_config.qcl_info = *CSIRSParamList.paramarray[0][10].uptr;
   } else {
     // 기본값 설정
     csirs_config.periodicity = 20;
     csirs_config.first_symbol = 13;
     csirs_config.power_offset = 0;
     csirs_config.density = 1;
     csirs_config.freq_start_rb = 0;
     csirs_config.freq_nrof_rbs = 106;
     csirs_config.freq_allocation[0] = 0;
     csirs_config.freq_allocation[1] = 16;
     strcpy(csirs_config.cdm_type, "fd_CDM2");
     csirs_config.nrof_ports = 2;
     csirs_config.qcl_info = 0;
   }
 ...
 /**
  * @brief SRS 상세 설정을 configuration에서 읽어옵니다
  * @param cfg Configuration 모듈 인터페이스
  * @param gnb_idx gNB 인덱스
  * @return SRS 상세 설정 구조체
  */
 srs_detailed_config_t read_srs_detailed_config(configmodule_interface_t *cfg, int gnb_idx)
 {
   srs_detailed_config_t srs_config = {0};
   char aprefix[MAX_OPTNAME_SIZE * 2 + 8];
   
   snprintf(aprefix, sizeof(aprefix), "%s.[%i]", GNB_CONFIG_STRING_GNB_LIST, gnb_idx);
   
   // SRS 기본 설정 읽기
   GET_PARAMS_LIST(SRSParamList, SRSParams, SRSPARAMS_DESC, GNB_CONFIG_STRING_SRS_DETAILED_CONFIG, aprefix);
   
   if (SRSParamList.numelt > 0) {
     srs_config.periodicity = *SRSParamList.paramarray[0][0].uptr;
     srs_config.start_position = *SRSParamList.paramarray[0][1].uptr;
     srs_config.num_symbols = *SRSParamList.paramarray[0][2].uptr;
     srs_config.repetition_factor = *SRSParamList.paramarray[0][3].uptr;
     srs_config.comb_offset = *SRSParamList.paramarray[0][4].uptr;
     srs_config.cyclic_shift = *SRSParamList.paramarray[0][5].uptr;
     srs_config.freq_domain_position = *SRSParamList.paramarray[0][6].uptr;
     srs_config.freq_domain_shift = *SRSParamList.paramarray[0][7].uptr;
     srs_config.freq_hopping_b_srs = *SRSParamList.paramarray[0][8].uptr;
     srs_config.freq_hopping_b_hop = *SRSParamList.paramarray[0][9].uptr;
     srs_config.freq_hopping_c_srs = *SRSParamList.paramarray[0][10].uptr;
     strcpy(srs_config.group_hopping, *SRSParamList.paramarray[0][11].strptr);
     strcpy(srs_config.alpha, *SRSParamList.paramarray[0][12].strptr);
     srs_config.p0 = *SRSParamList.paramarray[0][13].iptr;
     srs_config.nrof_srs_ports = *SRSParamList.paramarray[0][14].uptr;
   } else {
     // 기본값 설정
     srs_config.periodicity = 10;
     srs_config.start_position = 0;
     srs_config.num_symbols = 1;
     srs_config.repetition_factor = 1;
     srs_config.comb_offset = 0;
     srs_config.cyclic_shift = 0;
     srs_config.freq_domain_position = 0;
     srs_config.freq_domain_shift = 0;
     srs_config.freq_hopping_b_srs = 0;
     srs_config.freq_hopping_b_hop = 0;
     srs_config.freq_hopping_c_srs = 0;
     strcpy(srs_config.group_hopping, "neither");
     strcpy(srs_config.alpha, "alpha1");
     srs_config.p0 = -84;
     srs_config.nrof_srs_ports = 1;
   }
 ...
 /**
  * @brief 코드북 상세 설정을 configuration에서 읽어옵니다
  * @param cfg Configuration 모듈 인터페이스
  * @param gnb_idx gNB 인덱스
  * @return 코드북 상세 설정 구조체
  */
 static codebook_detailed_config_t read_codebook_detailed_config(configmodule_interface_t *cfg, int gnb_idx)
 {
   codebook_detailed_config_t codebook_config = {0};
   char aprefix[MAX_OPTNAME_SIZE * 2 + 8];
   
   snprintf(aprefix, sizeof(aprefix), "%s.[%i]", GNB_CONFIG_STRING_GNB_LIST, gnb_idx);
   
   // 코드북 기본 설정 읽기
   GET_PARAMS_LIST(CodebookParamList, CodebookParams, CODEBOOKPARAMS_DESC, GNB_CONFIG_STRING_CODEBOOK_DETAILED_CONFIG, aprefix);
   
   if (CodebookParamList.numelt > 0) {
     strcpy(codebook_config.codebook_type, *CodebookParamList.paramarray[0][0].strptr);
     strcpy(codebook_config.sub_type, *CodebookParamList.paramarray[0][1].strptr);
     codebook_config.mode = *CodebookParamList.paramarray[0][2].uptr;
     codebook_config.pmi_restriction = *CodebookParamList.paramarray[0][3].uptr;
     codebook_config.ri_restriction = *CodebookParamList.paramarray[0][4].uptr;
     strcpy(codebook_config.n1_n2_config, *CodebookParamList.paramarray[0][5].strptr);
     // Type-II Port Selection parameters (indices 6-9)
     strcpy(codebook_config.phase_alphabet, *CodebookParamList.paramarray[0][6].strptr);
     codebook_config.subband_amplitude = *CodebookParamList.paramarray[0][7].uptr;
     codebook_config.number_of_beams = *CodebookParamList.paramarray[0][8].uptr;
     codebook_config.port_selection_sampling_size = *CodebookParamList.paramarray[0][9].uptr;
   } else {
     strcpy(codebook_config.codebook_type, "type1");
     strcpy(codebook_config.sub_type, "typeI_SinglePanel");
     codebook_config.mode = 1;
     codebook_config.pmi_restriction = 0xff;
     codebook_config.ri_restriction = 0x3;
     strcpy(codebook_config.n1_n2_config, "two_one");
     strcpy(codebook_config.phase_alphabet, "n4");
     codebook_config.subband_amplitude = 0;
     codebook_config.number_of_beams = 2;
     codebook_config.port_selection_sampling_size = 2;
   }
   
   return codebook_config;
 }
 
 /**
  * @brief CSI 측정 설정을 configuration에서 읽어옵니다
  * @param cfg Configuration 모듈 인터페이스
  * @param gnb_idx gNB 인덱스
  * @return CSI 측정 설정 구조체
  */
 csi_measurement_config_t read_csi_measurement_config(configmodule_interface_t *cfg, int gnb_idx)
 {
   csi_measurement_config_t csi_config = {0};
   char aprefix[MAX_OPTNAME_SIZE * 2 + 8];
   
   snprintf(aprefix, sizeof(aprefix), "%s.[%i]", GNB_CONFIG_STRING_GNB_LIST, gnb_idx);
   
   // CSI 측정 기본 설정 읽기
   GET_PARAMS_LIST(CSIParamList, CSIParams, CSIPARAMS_DESC, GNB_CONFIG_STRING_CSI_MEASUREMENT_CONFIG, aprefix);
   
   if (CSIParamList.numelt > 0) {
     strcpy(csi_config.cqi_table, *CSIParamList.paramarray[0][0].strptr);
     strcpy(csi_config.ri_table, *CSIParamList.paramarray[0][1].strptr);
     strcpy(csi_config.pmi_table, *CSIParamList.paramarray[0][2].strptr);
     csi_config.report_periodicity = *CSIParamList.paramarray[0][3].uptr;
     csi_config.report_offset = *CSIParamList.paramarray[0][4].uptr;
     csi_config.cqi_threshold = *CSIParamList.paramarray[0][5].uptr;
     csi_config.ri_threshold = *CSIParamList.paramarray[0][6].uptr;
   } else {
     // 기본값 설정
     strcpy(csi_config.cqi_table, "table1");
     strcpy(csi_config.ri_table, "table1");
     strcpy(csi_config.pmi_table, "table1");
     csi_config.report_periodicity = 20;
     csi_config.report_offset = 0;
     csi_config.cqi_threshold = 0;
     csi_config.ri_threshold = 0;
   }
 ...
 /**
  * @brief 통합 상세 설정을 configuration에서 읽어옵니다
  * @param cfg Configuration 모듈 인터페이스
  * @param gnb_idx gNB 인덱스
  * @return 통합 상세 설정 구조체
  */
 nr_detailed_config_t read_nr_detailed_config(configmodule_interface_t *cfg, int gnb_idx)
 {
   nr_detailed_config_t detailed_config = {0};
   
   detailed_config.csirs = read_csirs_detailed_config(cfg, gnb_idx);
   detailed_config.srs = read_srs_detailed_config(cfg, gnb_idx);
   detailed_config.codebook = read_codebook_detailed_config(cfg, gnb_idx);
   detailed_config.csi_measurement = read_csi_measurement_config(cfg, gnb_idx);
