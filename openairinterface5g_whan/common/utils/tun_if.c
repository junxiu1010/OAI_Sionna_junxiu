...
static int get_gpu_ipc_ue_offset(void)
{
  const char *env = getenv("RFSIM_GPU_IPC_UE_IDX");
  return env ? atoi(env) : 0;
}

void setup_ue_ipv4_route(const char* ifname, int instance_id, const char *ipv4)
{
  int table_id = (instance_id + get_gpu_ipc_ue_offset()) - 1 + 10000;
...
{
  int effective_id = instance_id + get_gpu_ipc_ue_offset();
  char pdu_session_string[10];
  snprintf(pdu_session_string, sizeof(pdu_session_string), "p%d", pdu_session_id);
  return snprintf(ifname, IFNAMSIZ, "%s%d%s", "oaitun_ue", effective_id + 1, pdu_session_id == -1 ? "" : pdu_session_string);
}
