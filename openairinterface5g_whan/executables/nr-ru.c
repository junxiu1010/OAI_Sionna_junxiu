...
void *ru_thread(void *param)
{
  ...
  while (!oai_exit) {
    ...
    {
      int threshold = ru->sl_ahead - 2;
      if (threshold < 1) threshold = 1;
      int gap;
      while ((gap = __atomic_load_n(&gNB->ru_push_count, __ATOMIC_ACQUIRE)
                  - __atomic_load_n(&gNB->l1tx_done_count, __ATOMIC_ACQUIRE)) >= threshold) {
        usleep(200);
      }
    }
#endif
...
#ifdef USE_GPU_IPC_V2
    __atomic_fetch_add(&gNB->ru_push_count, 1, __ATOMIC_RELEASE);
#endif
  }
