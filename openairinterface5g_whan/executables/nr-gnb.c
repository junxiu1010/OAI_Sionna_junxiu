...
static void tx_func(processingData_L1tx_t *info)
{
  ...
  __atomic_fetch_add(&gNB->l1tx_done_count, 1, __ATOMIC_RELEASE);
#endif
  ...
  /* GPU IPC ring is a FIFO without timestamp indexing. If UL slots
   * have no ring entry, the UE reads the next DL slot's data at the
   * wrong slot position, permanently misaligning OFDM symbols.
   * Write zero-filled data so every slot has exactly one ring entry.
   * Only active when GPU IPC V2 is in use (checked at runtime). */
  else if (tx_slot_type == NR_UPLINK_SLOT && getenv("RFSIM_GPU_IPC_V2") && atoi(getenv("RFSIM_GPU_IPC_V2")) == 1) {
    processingData_RU_t syncMsgRU;
    syncMsgRU.frame_tx = frame_tx;
    syncMsgRU.slot_tx = slot_tx;
    syncMsgRU.ru = gNB->RU_list[0];
