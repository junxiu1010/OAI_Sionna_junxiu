...
int nr_fill_prach(PHY_VARS_gNB *gNB, int SFN, int Slot, nfapi_nr_prach_pdu_t *prach_pdu)
{
  ...
  if (prach_id < 0) {
    LOG_E(PHY, "[PRACH] no free gNB PRACH slot for %d.%d (all %d occupied), skipping\n",
...
void nr_fill_prach_ru(RU_t *ru, int SFN, int Slot, nfapi_nr_prach_pdu_t *prach_pdu, int *beam_id)
{
#ifdef USE_GPU_IPC_V2
  {
    int ru_frame = __atomic_load_n(&ru->proc.frame_rx, __ATOMIC_ACQUIRE);
    int ru_slot  = __atomic_load_n(&ru->proc.tti_rx, __ATOMIC_ACQUIRE);
    int spf = ru->nr_frame_parms->slots_per_frame;
    int total = 1024 * spf;
    int diff = (ru_frame * spf + ru_slot - SFN * spf - Slot + total) % total;
    if (diff > 0 && diff < total / 2) {
      LOG_W(PHY, "[PRACH-C] skip past slot %d.%d (RU at %d.%d, diff=%d)\n",
            SFN, Slot, ru_frame, ru_slot, diff);
      return;
    }
  }
#endif
  ...
  if (prach_id < 0) {
    ...
    for (int i = 0; i < NUMBER_OF_NR_RU_PRACH_MAX; i++) {
      ...
      if (p->frame < 0)
        continue;
      int entry_end = (p->frame * spf + p->slot + p->num_slots - 1) % total;
      int ru_abs    = (ru_frame * spf + ru_slot) % total;
      int age = (ru_abs - entry_end + total) % total;
      if (age > 0 && age < total / 2) {
        LOG_W(PHY, "[PRACH-GC] reclaim stale entry %d (%d.%d, num_slots=%d) RU at %d.%d\n",
              i, p->frame, p->slot, p->num_slots, ru_frame, ru_slot);
        p->frame = -1;
        p->slot  = -1;
        p->num_slots = -1;
        reclaimed++;
    ...
    pthread_mutex_unlock(&ru->prach_list_mutex);
    if (reclaimed > 0) {
      LOG_W(PHY, "[PRACH-GC] reclaimed %d stale entries, retrying\n", reclaimed);
      prach_id = find_nr_prach_ru(ru, SFN, Slot, SEARCH_EXIST_OR_FREE);
    }
  }
  if (prach_id < 0) {
    LOG_E(PHY, "[PRACH] no free RU PRACH slot for %d.%d (all %d occupied), skipping\n",
          SFN, Slot, NUMBER_OF_NR_RU_PRACH_MAX);
    return;
  }
#else
  AssertFatal((prach_id >= 0) && (prach_id < NUMBER_OF_NR_RU_PRACH_MAX),
