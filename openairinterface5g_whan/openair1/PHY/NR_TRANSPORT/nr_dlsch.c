...
// Helper: element-wise add src into dst for count c16_t samples
static inline void c16_add_array(c16_t *dst, const c16_t *src, int count)
{
                              ...
                              int txdataF_offset_per_symbol)
{
  ...
  const int add_mode = rel15->mu_mimo_secondary; // MU-MIMO: ADD to existing txdataF
  int rb = 0;
    ...
    if (pmi == 0) { // unitary Precoding
      if (subCarrier + re_cnt <= symbol_sz) { // RB does not cross DC
        if (ant < rel15->nrOfLayers) {
          if (add_mode)
            c16_add_array(&txdataF[ant][txdataF_offset_per_symbol + subCarrier],
                          &txdataF_precoding[ant][subCarrier], re_cnt);
          else
        ...
        if (ant < rel15->nrOfLayers) {
          if (add_mode) {
            c16_add_array(&txdataF[ant][txdataF_offset_per_symbol + subCarrier],
                          &txdataF_precoding[ant][subCarrier], neg_length);
            c16_add_array(&txdataF[ant][txdataF_offset_per_symbol],
                          &txdataF_precoding[ant][0], pos_length);
          } else {
            memcpy(&txdataF[ant][txdataF_offset_per_symbol + subCarrier],
                   &txdataF_precoding[ant][subCarrier],
                   neg_length * sizeof(**txdataF));
            memcpy(&txdataF[ant][txdataF_offset_per_symbol], &txdataF_precoding[ant][0], pos_length * sizeof(**txdataF));
          }
        } else if (!add_mode) {
      ...
      if ((subCarrier + re_cnt) < symbol_sz) { // within ofdm_symbol_size, use SIMDe
        if (add_mode)
          nr_layer_precoder_simd_add(rel15->nrOfLayers,
                                     symbol_sz,
                                     txdataF_precoding,
                                     ant,
                                     pmi_pdu,
        ...
        for (int i = 0; i < re_cnt; i++) {
          c16_t precoded = nr_layer_precoder_cm(rel15->nrOfLayers, symbol_sz, txdataF_precoding, ant, pmi_pdu, subCarrier);
          if (add_mode) {
            txdataF[ant][txdataF_offset_per_symbol + subCarrier].r += precoded.r;
            txdataF[ant][txdataF_offset_per_symbol + subCarrier].i += precoded.i;
          } else {
            txdataF[ant][txdataF_offset_per_symbol + subCarrier] = precoded;
          }
#ifdef DEBUG_DLSCH_MAPPING
...
static int do_one_dlsch(unsigned char *input_ptr, PHY_VARS_gNB *gNB, NR_gNB_DLSCH_t *dlsch, int slot)
{
  ...
  // MU-MIMO: apply power scaling alpha to amplitude
  int16_t amp = gNB->TX_AMP;
  if (rel15->mu_mimo_alpha_q15 > 0 && rel15->mu_mimo_alpha_q15 < 32767) {
    amp = (int16_t)(((int32_t)amp * rel15->mu_mimo_alpha_q15) >> 15);
  }
